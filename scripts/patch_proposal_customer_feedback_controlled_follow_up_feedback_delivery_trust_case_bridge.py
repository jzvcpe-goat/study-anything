#!/usr/bin/env python3
"""Bridge controlled follow-up feedback candidates into Delivery Trust Case refs.

This layer consumes the metadata-only
`patch-proposal-delivery-trust-case-candidate-v1` emitted by the controlled
follow-up feedback Delivery Trust Intake Gate. It then runs the existing
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_intake_gate as intake_gate  # noqa: E402


RECEIPT_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt-v1"
HANDOFF_REFS_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-feedback-delivery-trust-case-handoff-refs-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-case-bridge-v1"
...
CLAIM_BOUNDARY = {
    "current_claim": (
        ...
        "candidate can be assembled into metadata-only Delivery Trust case and "
        "handoff refs only when the Product Loop, controlled follow-up feedback, "
        "controlled-failure, active attention reconstruction, Dual Loop gate, "
        "and local deterministic Delivery Trust Case Harness evidence all match."
    ),
    "not_claimed": [
...
class PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(ValueError):
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(f"Expected JSON object: {path}")
    return payload
...
def _require_object(payload: Mapping[str, Any], key: str, *, label: str) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(f"{label}.{key} must be an object")
    return value
...
def _validate_privacy(payload: Mapping[str, Any], *, label: str) -> None:
    ...
    if forbidden:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(
    ...
    refs = {
        **_base(HANDOFF_REFS_SCHEMA_VERSION),
        ...
        "candidate_ref": _artifact_ref(
            ...
            f"fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-intake-gate/{case_id}/patch-proposal-delivery-trust-case-candidate.json",
            candidate,
        ),
        ...
        "controlled_follow_up_evidence_chain": {
            "source_intake_gate": "controlled_follow_up_feedback_delivery_trust_intake_gate",
            "source_product_loop_chain": "controlled_follow_up_feedback_product_loop_brief_intake_gate",
            "product_loop_run_ref": "product-loop-run.json",
            "delivery_trust_case_harness_evidence_present": True,
            "active_reconstruction_required": True,
            "passive_attention_only_sufficient": False,
            "raw_controlled_follow_up_bodies_included": False,
        },
...
def validate_handoff_refs(refs: Mapping[str, Any]) -> dict[str, Any]:
    if refs.get("schema_version") != HANDOFF_REFS_SCHEMA_VERSION:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("handoff refs schema_version drifted")
    ...
    if candidate_ref.get("raw_body_included") is not False:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("candidate ref must not include raw body")
    for key in ("source_evidence_refs", "delivery_trust_refs"):
        values = refs.get(key)
        if not isinstance(values, list) or not values:
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(f"handoff refs missing {key}")
        for item in values:
            if not isinstance(item, Mapping):
                raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(f"{key} item must be object")
            if item.get("raw_body_included") is not False:
                raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(f"{key} must not include raw bodies")
    ...
    if summary.get("status") != "ready_for_controlled_customer_handoff":
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("handoff refs require ready Delivery Trust Case")
    if summary.get("customer_visible_send_performed") is not False:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("handoff refs must not send customers")
    chain = _require_object(refs, "controlled_follow_up_evidence_chain", label=HANDOFF_REFS_SCHEMA_VERSION)
    if chain.get("delivery_trust_case_harness_evidence_present") is not True:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("handoff refs require Delivery Trust Case Harness evidence")
    if chain.get("active_reconstruction_required") is not True:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("handoff refs require active reconstruction")
    if chain.get("passive_attention_only_sufficient") is not False:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("passive attention alone must not be sufficient")
    if chain.get("raw_controlled_follow_up_bodies_included") is not False:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("controlled follow-up bodies must not be included")
    return dict(refs)
    ...
    *,
    ...
    receipt = {
        **_base(RECEIPT_SCHEMA_VERSION),
        "bridge_receipt_id": (
            f"patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-{candidate_hash[:16]}"
            if candidate_hash
            else f"patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-blocked-{case_id}"
        ),
        "case_id": case_id,
        ...
        "decision": (
            "emit_patch_proposal_controlled_follow_up_feedback_delivery_trust_case_handoff_refs"
            if ready
            else "block_patch_proposal_controlled_follow_up_feedback_delivery_trust_case_bridge"
        ),
        "blocked_reasons": reasons,
        "source_refs": {
            "candidate_ref": (
                f"fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-intake-gate/"
        ...
        "harness_policy": {
            ...
            "emit_refs_only": True,
            "controlled_follow_up_feedback_chain_required": True,
            "delivery_trust_case_evidence_required": True,
            "active_reconstruction_required": True,
...
def validate_bridge_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    if receipt.get("schema_version") != RECEIPT_SCHEMA_VERSION:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("receipt schema_version drifted")
    ...
    if status not in {"delivery_trust_case_refs_ready", "blocked"}:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("bridge receipt status is invalid")
    if not isinstance(reasons, list):
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("bridge receipt blocked_reasons must be a list")
    refs = receipt.get("handoff_refs")
    if status == "delivery_trust_case_refs_ready":
        if decision != "emit_patch_proposal_controlled_follow_up_feedback_delivery_trust_case_handoff_refs":
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("ready receipt decision drifted")
        if reasons:
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("ready receipt must not include blocked reasons")
        if not isinstance(refs, Mapping):
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("ready receipt must include handoff refs")
        validate_handoff_refs(refs)
    else:
        if decision != "block_patch_proposal_controlled_follow_up_feedback_delivery_trust_case_bridge":
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("blocked receipt decision drifted")
        if not reasons:
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("blocked receipt must include reasons")
        if refs is not None:
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("blocked receipt must not include handoff refs")
    ...
    for key in (
        ...
        if policy.get(key) is not False:
            raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(f"harness_policy.{key} must be false")
    if policy.get("emit_refs_only") is not True:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("bridge must emit refs only")
    if policy.get("controlled_follow_up_feedback_chain_required") is not True:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("controlled follow-up evidence chain is required")
    if policy.get("delivery_trust_case_evidence_required") is not True:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("Delivery Trust Case evidence is required")
    if policy.get("active_reconstruction_required") is not True:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError("active reconstruction is required")
    return dict(receipt)


def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    if case_id not in CASE_IDS:
        raise PatchProposalControlledFollowUpFeedbackDeliveryTrustCaseBridgeError(f"Unknown case id: {case_id}")
    ...
    artifacts = {"patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json": receipt}
    refs = receipt.get("handoff_refs")
    if isinstance(refs, Mapping):
        artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-handoff-refs.json"] = validate_handoff_refs(refs)
    return artifacts
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_bridge_receipt(cases[case_id]["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json"])
        case_reports.append(
            {
                "case_id": case_id,
    ...
    report = {
        **_base(REPORT_SCHEMA_VERSION),
        "status": "pass",
        "purpose": (
            "Prove controlled follow-up feedback Patch Proposal Delivery Trust case candidates can invoke "
            "the local deterministic Delivery Trust Case Harness only after candidate, Product Loop, "
            "controlled follow-up feedback, controlled-failure, active attention-reconstruction, and Dual "
            "Loop refs match, while emitting refs only."
        ),
...
def main() -> int:
    ...
    if args.candidate:
        ...
        selected: dict[str, dict[str, dict[str, Any]]] = {
            "custom": {"patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json": receipt}
        }
        refs = receipt.get("handoff_refs")
        if isinstance(refs, Mapping):
            selected["custom"]["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-handoff-refs.json"] = validate_handoff_refs(refs)
    else:
    ...
    print(
        dump_json(
            {
                "schema_version": "patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-cli-result-v1",
                "status": "ok",
                "case_ids": list(selected),
                "ready_case_count": sum(
                    1
                    for artifacts in selected.values()
                    if artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json"]["status"]
                    == "delivery_trust_case_refs_ready"
                ),
                "blocked_case_count": sum(
                    1
                    for artifacts in selected.values()
                    if artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json"]["status"] == "blocked"
                ),
                "model_calls_performed": False,
