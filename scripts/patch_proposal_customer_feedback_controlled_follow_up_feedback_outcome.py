#!/usr/bin/env python3
"""Build metadata-only Patch Proposal controlled follow-up feedback outcome receipts."""
...
OUTCOME_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-feedback-outcome-receipt-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-controlled-follow-up-feedback-outcome-v1"
...
REHEARSAL_FIXTURE_DIR = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-rehearsal"
...
PRIVACY_FLAGS = {
    **rehearsal.PRIVACY_FLAGS,
    "controlled_follow_up_feedback_outcome_metadata_only": True,
    "follow_up_feedback_outcome_receipt_only": True,
...
CLAIM_BOUNDARY = {
    "current_claim": (
        "A controlled follow-up feedback outcome receipt records only metadata that a "
...
class PatchProposalControlledFollowUpOutcomeError(ValueError):
    """Raised when controlled follow-up feedback outcome artifacts are unsafe."""
...
def source_rehearsal(case_id: str) -> dict[str, Any]:
    ...
    receipt = load_json(
        REHEARSAL_FIXTURE_DIR / rehearsal_case / "patch-proposal-controlled-follow-up-feedback-rehearsal-receipt.json"
    )
...
def _action_ref_hash(case_id: str, actor_type: str | None) -> str | None:
    if not _case_flags(case_id)["external_action_ref_hash_present"]:
        return None
    return dual_loop.sha256_text(f"{case_id}:{actor_type}:external-controlled-follow-up-feedback-outcome")


def build_controlled_follow_up_feedback_outcome(case_id: str) -> dict[str, Any]:
    ...
    receipt = {
        **_base(OUTCOME_SCHEMA_VERSION),
        "outcome_id": f"patch-proposal-controlled-follow-up-feedback-outcome-{case_id}",
        "case_id": case_id,
        "status": "recorded" if recorded else "blocked",
        "decision": "record_external_controlled_follow_up_feedback_outcome" if recorded else "block_controlled_follow_up_feedback_outcome",
        "blocked_reasons": reasons,
        "source_refs": {
            "rehearsal_receipt_ref": (
                "fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-rehearsal/"
                f"{flags['rehearsal_case']}/patch-proposal-controlled-follow-up-feedback-rehearsal-receipt.json"
            ),
        ...
        "external_follow_up_outcome": {
            "package_type": "metadata_only_patch_proposal_controlled_follow_up_feedback_outcome",
    ...
    return validate_controlled_follow_up_feedback_outcome(receipt)


def validate_controlled_follow_up_feedback_outcome(receipt: Mapping[str, Any]) -> dict[str, Any]:
    if receipt.get("schema_version") != OUTCOME_SCHEMA_VERSION:
        raise PatchProposalControlledFollowUpOutcomeError("controlled follow-up feedback outcome schema_version drifted")
    ...
    if summary.get("package_type") != "metadata_only_patch_proposal_controlled_follow_up_feedback_outcome":
    ...
    if status == "recorded":
        if receipt.get("blocked_reasons") != []:
            raise PatchProposalControlledFollowUpOutcomeError("recorded controlled follow-up feedback outcome must not carry reasons")
        for key in (
        ...
        if not receipt.get("blocked_reasons"):
            raise PatchProposalControlledFollowUpOutcomeError("blocked controlled follow-up feedback outcome must carry reasons")
        if summary.get("outcome_type") is not None:
            raise PatchProposalControlledFollowUpOutcomeError("blocked controlled follow-up feedback outcome must not expose outcome_type")
    else:
        raise PatchProposalControlledFollowUpOutcomeError("controlled follow-up feedback outcome status must be recorded or blocked")
    return dict(receipt)


def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    return {
        "patch-proposal-controlled-follow-up-feedback-outcome-receipt.json": build_controlled_follow_up_feedback_outcome(
            case_id
        )
    }
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_controlled_follow_up_feedback_outcome(
            cases[case_id]["patch-proposal-controlled-follow-up-feedback-outcome-receipt.json"]
        )
        case_reports.append(
            {
                "case_id": case_id,
    ...
    report = {
        **_base(REPORT_SCHEMA_VERSION),
        "status": "pass",
        "purpose": (
            "Prove external controlled follow-up feedback outcomes can be recorded as metadata-only receipts "
        ...
        "source_chain": {
            "ready_rehearsal_receipt": (
                "fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-rehearsal/"
                "pass-operator-signal/patch-proposal-controlled-follow-up-feedback-rehearsal-receipt.json"
            ),
            "rehearsal_report": (
                "platform/generated/"
                "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-rehearsal.json"
