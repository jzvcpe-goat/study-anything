#!/usr/bin/env python3
"""Verify Patch Proposal Customer Feedback Controlled Follow-up Feedback Delivery Trust Case Bridge artifacts."""
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_case_bridge as bridge  # noqa: E402


FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-case-bridge"
REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-case-bridge.json"
MARKDOWN_REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-case-bridge.md"
HTML_REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-case-bridge.html"
REPORT_SCHEMA_FILE = ROOT / "platform" / "schemas" / "cbb" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-case-bridge-v1.schema.json"
RECEIPT_SCHEMA_FILE = ROOT / "platform" / "schemas" / "cbb" / "patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt-v1.schema.json"
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    receipt = bridge.validate_bridge_receipt(artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json"])
    ...
    if refs_created:
        refs = bridge.validate_handoff_refs(artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-handoff-refs.json"])
    ...
    dual_loop.assert_metadata_only(
        receipt,
        label=f"patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge:{case_id}",
    )
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-delivery-trust-case-bridge-") as tmp:
        ...
        proc = subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_case_bridge.py"),
                "--case",
                "all",
                "--output-dir",
                str(output_dir),
        ...
        dual_loop.assert_metadata_only(
            stdout,
            label="patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-cli",
        )
        if stdout.get("case_ids") != list(bridge.CASE_IDS):
            raise RuntimeError("Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge CLI case order drifted")
        for case_id, expected_artifacts in cases.items():
            for filename, expected in expected_artifacts.items():
                ...
                if not path.is_file():
                    raise RuntimeError(f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge CLI missing {case_id}/{filename}")
                if load_json(path) != expected:
                    raise RuntimeError(
                        f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge CLI output drifted: {case_id}/{filename}"
                    )
            refs_path = output_dir / case_id / "patch-proposal-controlled-follow-up-feedback-delivery-trust-case-handoff-refs.json"
            if expected_artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json"]["status"] == "blocked":
                if refs_path.exists():
        ...
        if custom_stdout.get("case_ids") != ["custom"]:
            raise RuntimeError("Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge custom CLI output drifted")
...
def negative_fixtures(pass_receipt: Mapping[str, Any]) -> dict[str, str]:
    ...
    expect_rejected(
        "external_publication_policy_injected",
        lambda payload: payload["harness_policy"].__setitem__("external_publication_allowed", True),
    )
    expect_rejected(
        "missing_controlled_follow_up_chain",
        lambda payload: payload["handoff_refs"].__setitem__("controlled_follow_up_evidence_chain", {}),
    )
    expect_rejected(
        "passive_attention_only_injected",
        lambda payload: payload["handoff_refs"]["controlled_follow_up_evidence_chain"].__setitem__(
            "passive_attention_only_sufficient",
            True,
        ),
    )
    expect_rejected(
        "delivery_trust_case_evidence_missing",
        lambda payload: payload["handoff_refs"]["controlled_follow_up_evidence_chain"].__setitem__(
            "delivery_trust_case_harness_evidence_present",
            False,
        ),
    )
    expect_rejected(
    ...
    expect_rejected(
        ...
        lambda payload: (
            ...
            payload.__setitem__("decision", "block_patch_proposal_controlled_follow_up_feedback_delivery_trust_case_bridge"),
    ...
    if missing:
        raise RuntimeError(f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge negative checks missing: {missing}")
    return failures
...
def build_report() -> dict[str, Any]:
    ...
    if report["case_reports"] != case_reports:
        raise RuntimeError("Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge case summary drifted")
    report["schema_files"] = [
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-customer-signal"]["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json"]
    )
...
def check_fixtures(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    for case_id, artifacts in cases.items():
        ...
        for filename, payload in artifacts.items():
            ...
            if not path.is_file():
                raise SystemExit(f"Missing Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge fixture: {path}")
            if load_json(path) != payload:
                raise SystemExit(
                    f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge fixture is out of date: {path}. "
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_case_bridge.py --write"
                )
        refs_path = case_dir / "patch-proposal-controlled-follow-up-feedback-delivery-trust-case-handoff-refs.json"
        if (
            artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-case-bridge-receipt.json"]["status"] == "blocked"
            and refs_path.exists()
        ):
            raise SystemExit(f"Blocked Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge case must not include refs: {case_id}")
...
def main() -> int:
    ...
    html = dual_loop.render_html_report("Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge", report)
    ...
    if args.write:
        ...
        print("wrote Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge fixtures and reports")
        return 0
    ...
    if args.check:
        ...
        if not REPORT.is_file() or REPORT.read_text(encoding="utf-8") != report_text:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge report is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_case_bridge.py --write"
            )
        if not MARKDOWN_REPORT.is_file() or MARKDOWN_REPORT.read_text(encoding="utf-8") != md:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge markdown is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_case_bridge.py --write"
            )
        if not HTML_REPORT.is_file() or HTML_REPORT.read_text(encoding="utf-8") != html:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge HTML is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_case_bridge.py --write"
            )
        print("ok    Patch Proposal Controlled Follow-up Feedback Delivery Trust Case Bridge report is up to date")
        return 0
