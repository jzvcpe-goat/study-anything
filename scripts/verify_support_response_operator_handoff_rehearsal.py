#!/usr/bin/env python3
"""Verify a metadata-only operator handoff rehearsal for Support Response delivery."""
...
SUPPORT_RESPONSE = ROOT / "platform" / "generated" / "study-anything-support-response-delivery-class.json"
CUSTOMER_REHEARSAL = ROOT / "platform" / "generated" / "study-anything-customer-delivery-rehearsal.json"
REPORT = ROOT / "platform" / "generated" / "study-anything-support-response-operator-handoff-rehearsal.json"
MARKDOWN_REPORT = ROOT / "platform" / "generated" / "study-anything-support-response-operator-handoff-rehearsal.md"
...
READY_CONFIRMATIONS = (
    "support_response_delivery_class_ready",
    ...
    "no_raw_response_or_ticket_payload_attached",
...
SUPPORT_RESPONSE_SOURCE_NOT_CLAIMED = {
    "automatic support reply delivery",
    "support resolution guarantee",
    "private requester identity disclosure",
    "legal certification",
...
class SupportResponseOperatorHandoffRehearsalError(RuntimeError):
    """Readable support-response operator handoff rehearsal verifier failure."""
...
def reject_forbidden_text(payload: Any, label: str) -> None:
    ...
    if hits:
        raise SupportResponseOperatorHandoffRehearsalError(f"{label} contains forbidden private text: {hits}")


def assert_false(mapping: Mapping[str, Any], keys: tuple[str, ...], label: str) -> None:
    for key in keys:
        if mapping.get(key) is not False:
            raise SupportResponseOperatorHandoffRehearsalError(f"{label}.{key} must be false")
...
def assert_absent_or_false(mapping: Mapping[str, Any], keys: tuple[str, ...], label: str) -> None:
    for key in keys:
        if key in mapping and mapping.get(key) is not False:
            raise SupportResponseOperatorHandoffRehearsalError(f"{label}.{key} must be absent or false")
...
def require_not_claimed(claim_boundary: Mapping[str, Any], label: str, required: set[str]) -> None:
    ...
    if missing:
        raise SupportResponseOperatorHandoffRehearsalError(f"{label} claim boundary missing not_claimed items: {missing}")


def validate_support_response_source(source: Mapping[str, Any]) -> dict[str, Any]:
    if source.get("schema_version") != "support-response-delivery-class-verification-v1":
        raise SupportResponseOperatorHandoffRehearsalError("support-response source schema_version drifted")
    if source.get("status") != "pass" or source.get("delivery_class") != "support_response_handoff":
        raise SupportResponseOperatorHandoffRehearsalError("support-response source must pass for support_response_handoff")
    privacy = source.get("privacy")
    if not isinstance(privacy, Mapping) or privacy.get("metadata_only") is not True:
        raise SupportResponseOperatorHandoffRehearsalError("support-response source privacy must be metadata-only")
    assert_absent_or_false(privacy, FALSE_PRIVACY_KEYS, "support_response_source.privacy")
    runtime = source.get("runtime")
    if not isinstance(runtime, Mapping):
        raise SupportResponseOperatorHandoffRehearsalError("support-response source missing runtime")
    assert_absent_or_false(
        ...
        "support_response_source.runtime",
    )
    claim_boundary = source.get("claim_boundary")
    if not isinstance(claim_boundary, Mapping):
        raise SupportResponseOperatorHandoffRehearsalError("support-response source missing claim_boundary")
    require_not_claimed(claim_boundary, "support-response source", SUPPORT_RESPONSE_SOURCE_NOT_CLAIMED)

    cases = source.get("case_reports")
    if not isinstance(cases, list) or not cases:
        raise SupportResponseOperatorHandoffRehearsalError("support-response source missing case reports")
    ready_cases = [
        case
        for case in cases
        if isinstance(case, Mapping) and case.get("decision") == "allow_controlled_support_response_handoff"
    ]
    blocked_cases = [
        case
        for case in cases
        if isinstance(case, Mapping) and case.get("decision") == "block_support_response_handoff"
    ]
    if len(ready_cases) != 1 or len(blocked_cases) < 4:
        raise SupportResponseOperatorHandoffRehearsalError("support-response source must include one ready and at least four blocked cases")
    reject_forbidden_text(source, "support-response source")
    return {
...
def validate_customer_rehearsal_source(source: Mapping[str, Any]) -> dict[str, Any]:
    if source.get("schema_version") != "customer-delivery-rehearsal-v1":
        raise SupportResponseOperatorHandoffRehearsalError("customer rehearsal source schema_version drifted")
    if source.get("status") != "pass":
        raise SupportResponseOperatorHandoffRehearsalError("customer rehearsal source must pass")
    privacy = source.get("privacy")
    if not isinstance(privacy, Mapping) or privacy.get("metadata_only") is not True:
        raise SupportResponseOperatorHandoffRehearsalError("customer rehearsal source privacy must be metadata-only")
    assert_absent_or_false(privacy, FALSE_PRIVACY_KEYS, "customer_rehearsal_source.privacy")
    claim_boundary = source.get("claim_boundary")
    if not isinstance(claim_boundary, Mapping):
        raise SupportResponseOperatorHandoffRehearsalError("customer rehearsal source missing claim_boundary")
    ...
    if not isinstance(rehearsal, Mapping):
        raise SupportResponseOperatorHandoffRehearsalError("customer rehearsal source missing rehearsal")
    ...
    if not isinstance(cases, list) or rehearsal.get("ready_count") != 1 or int(rehearsal.get("blocked_count", 0)) < 5:
        raise SupportResponseOperatorHandoffRehearsalError("customer rehearsal source must include one ready and at least five blocked cases")
    ready_cases = [
        case
        for case in cases
    ...
    if len(ready_cases) != 1 or len(blocked_cases) < 5:
        raise SupportResponseOperatorHandoffRehearsalError("customer rehearsal cases drifted")
...
def rehearsal_case(
    *,
    case_id: str,
    support_response_case_ref: str,
    customer_rehearsal_case_ref: str,
    requested_action: str,
    confirmations: Mapping[str, bool],
    support_response_blocked: bool = False,
    customer_rehearsal_blocked: bool = False,
    raw_payload_requested: bool = False,
) -> dict[str, Any]:
    reasons: list[str] = []
    if support_response_blocked:
        reasons.append("support_response_delivery_class_blocked")
    ...
    if raw_payload_requested:
        reasons.append("raw_response_or_ticket_payload_requested")
    ...
    return {
        ...
        "support_response_case_ref": support_response_case_ref,
        ...
        "raw_response_or_ticket_payload_included": False,
        "requires_human_external_action_outside_system": True,
    }


def build_report(support_response_path: Path, customer_rehearsal_path: Path) -> dict[str, Any]:
    support_response = load_json(support_response_path)
    customer_rehearsal = load_json(customer_rehearsal_path)
    support_refs = validate_support_response_source(support_response)
    customer_refs = validate_customer_rehearsal_source(customer_rehearsal)

    confirmations = {key: True for key in READY_CONFIRMATIONS}
    cases = [
        rehearsal_case(
            case_id="ready-support-response-operator-decision",
            support_response_case_ref=str(support_refs["ready_case"]["case_id"]),
            customer_rehearsal_case_ref=str(customer_refs["ready_case"]["case_id"]),
            requested_action="prepare_operator_handoff_decision",
            confirmations=confirmations,
        ),
        rehearsal_case(
            case_id="block-support-response-class-blocked",
            support_response_case_ref=str(support_refs["blocked_case"]["case_id"]),
            customer_rehearsal_case_ref=str(customer_refs["ready_case"]["case_id"]),
            requested_action="prepare_operator_handoff_decision",
            confirmations=confirmations,
            support_response_blocked=True,
        ),
        rehearsal_case(
        ...
        rehearsal_case(
            case_id="block-missing-support-policy-confirmation",
            support_response_case_ref=str(support_refs["ready_case"]["case_id"]),
            customer_rehearsal_case_ref=str(customer_refs["ready_case"]["case_id"]),
            requested_action="prepare_operator_handoff_decision",
            confirmations={**confirmations, "support_policy_scope_bounded": False},
        ),
        rehearsal_case(
        ...
        rehearsal_case(
            case_id="block-raw-ticket-or-response-requested",
            support_response_case_ref=str(support_refs["ready_case"]["case_id"]),
            customer_rehearsal_case_ref=str(customer_refs["ready_case"]["case_id"]),
            requested_action="prepare_operator_handoff_decision",
            confirmations={**confirmations, "no_raw_response_or_ticket_payload_attached": False},
    ...
    report = {
        ...
        "status": "pass",
        "purpose": (
            "Rehearse a Support Response delivery-class operator handoff decision by combining the "
            "Support Response Delivery Class report and the Customer Delivery Rehearsal, without "
            "sending customer messages, publishing externally, mutating production, or reading raw payloads."
        ),
        "sources": {
            "support_response_delivery_class": {
                "report": str(support_response_path.relative_to(ROOT)),
                "schema_version": support_response["schema_version"],
                "status": support_response["status"],
                "ready_case": support_refs["ready_case"]["case_id"],
                "blocked_case_count": support_refs["blocked_case_count"],
            },
        ...
        "rehearsal": {
            ...
            "operator_exit_criteria": [
                ...
                "raw_support_ticket_response_and_identity_are_absent",
                "actual_external_delivery_happens_outside_this_system_only",
            ],
        },
        "claim_boundary": {
            "current_claim": (
                "A platform Agent or external operator can prepare a Support Response handoff decision from "
                "metadata-only evidence when support policy scope, requester scope, and customer-delivery rehearsal pass."
            ),
            "not_claimed": [
                "customer send approval",
                "actual customer send completed",
        ...
        "acceptance": {
            "minimum_command": "python3 scripts/verify_support_response_operator_handoff_rehearsal.py --check",
            "source_commands": [
...
def validate_report(report: Mapping[str, Any]) -> None:
    if report.get("schema_version") != SCHEMA_VERSION or report.get("status") != "pass":
        raise SupportResponseOperatorHandoffRehearsalError("rehearsal report schema/status drifted")
    rehearsal = report.get("rehearsal")
    if not isinstance(rehearsal, Mapping):
        raise SupportResponseOperatorHandoffRehearsalError("report missing rehearsal")
    if rehearsal.get("delivery_class") != "support_response_handoff":
        raise SupportResponseOperatorHandoffRehearsalError("rehearsal delivery_class drifted")
    cases = rehearsal.get("cases")
    if not isinstance(cases, list) or not cases:
        raise SupportResponseOperatorHandoffRehearsalError("rehearsal must include cases")
    if rehearsal.get("ready_count") != 1:
        raise SupportResponseOperatorHandoffRehearsalError("rehearsal must include exactly one ready case")
    if int(rehearsal.get("blocked_count", 0)) < 7:
        raise SupportResponseOperatorHandoffRehearsalError("rehearsal must include at least seven blocked cases")
    for case in cases:
        if not isinstance(case, Mapping):
            raise SupportResponseOperatorHandoffRehearsalError("rehearsal case must be an object")
        assert_false(
            case,
            (
                "automatic_customer_sending_performed",
        ...
        if case.get("requires_human_external_action_outside_system") is not True:
            raise SupportResponseOperatorHandoffRehearsalError(
        ...
        if case.get("status") == "ready":
            ...
            if not isinstance(confirmations, Mapping) or any(confirmations.get(key) is not True for key in READY_CONFIRMATIONS):
                raise SupportResponseOperatorHandoffRehearsalError("ready case must include all active confirmations")
            if case.get("decision") != "ready_for_operator_handoff_decision":
                raise SupportResponseOperatorHandoffRehearsalError("ready case decision drifted")
        elif case.get("status") == "blocked":
            if case.get("decision") != "block_operator_handoff":
                raise SupportResponseOperatorHandoffRehearsalError("blocked case decision drifted")
            if int(case.get("reason_count", 0)) < 1:
                raise SupportResponseOperatorHandoffRehearsalError("blocked case must include at least one reason")
        else:
            raise SupportResponseOperatorHandoffRehearsalError(f"unknown case status: {case.get('status')}")

    claim = report.get("claim_boundary")
    if not isinstance(claim, Mapping):
        raise SupportResponseOperatorHandoffRehearsalError("report missing claim_boundary")
    require_not_claimed(claim, "report", FINAL_NOT_CLAIMED)
    privacy = report.get("privacy")
    if not isinstance(privacy, Mapping) or privacy.get("metadata_only") is not True:
        raise SupportResponseOperatorHandoffRehearsalError("rehearsal privacy must be metadata-only")
...
def negative_case(case_id: str, report: Mapping[str, Any], mutator: Any) -> dict[str, str]:
    ...
    except SupportResponseOperatorHandoffRehearsalError as exc:
        return {"case_id": case_id, "status": "rejected", "error": str(exc)}
    raise SupportResponseOperatorHandoffRehearsalError(f"Negative rehearsal fixture unexpectedly passed: {case_id}")
...
def negative_fixtures() -> list[dict[str, str]]:
    ...
    def strip_support_policy_scope(payload: dict[str, Any]) -> None:
        payload["rehearsal"]["cases"][0]["operator_confirmation_inputs"]["support_policy_scope_bounded"] = False
    ...
    def leak_raw_response(payload: dict[str, Any]) -> None:
        payload["privacy"]["raw_response_text_included"] = True
    ...
    return [
        ...
        negative_case("missing_support_policy_scope_confirmation", base, strip_support_policy_scope),
        negative_case("blocked_case_without_reason", base, unblock_without_reason),
        negative_case("raw_response_text_included", base, leak_raw_response),
...
def markdown_report(report: Mapping[str, Any]) -> str:
    ...
    return f"""# Support Response Operator Handoff Rehearsal
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--support-response-source", type=Path, default=SUPPORT_RESPONSE)
...
def main() -> None:
    args = parse_args()
    report = build_report(args.support_response_source, args.customer_rehearsal_source)
    ...
    if args.check:
        if not REPORT.exists() or REPORT.read_text(encoding="utf-8") != text:
            raise SupportResponseOperatorHandoffRehearsalError(
                "Support Response operator handoff rehearsal report is stale. Run: "
        ...
        if not MARKDOWN_REPORT.exists() or MARKDOWN_REPORT.read_text(encoding="utf-8") != markdown:
            raise SupportResponseOperatorHandoffRehearsalError(
                "Support Response operator handoff rehearsal markdown is stale. Run: "
                "python3 scripts/verify_support_response_operator_handoff_rehearsal.py --write"
            )
        print("ok    Support Response operator handoff rehearsal is up to date")
        return
    print(text, end="")
...
if __name__ == "__main__":
    try:
        ...
        print(f"verify_support_response_operator_handoff_rehearsal failed: {exc}", file=sys.stderr)
        sys.exit(1)
