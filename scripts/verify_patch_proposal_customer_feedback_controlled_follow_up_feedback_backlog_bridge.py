#!/usr/bin/env python3
"""Verify controlled follow-up feedback backlog bridge artifacts."""
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_backlog_bridge as bridge  # noqa: E402


FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge"
REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge.json"
)
MARKDOWN_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge.md"
)
HTML_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge.html"
)
BRIDGE_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge-v1.schema.json"
)
...
EXPECTED = {
    ...
    "pass-host-platform-agent-signal": (
        ...
        "emit_product_loop_backlog_signal",
        [],
        True,
    ),
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    payload = bridge.validate_controlled_follow_up_feedback_backlog_bridge(
        artifacts["patch-proposal-controlled-follow-up-feedback-backlog-bridge.json"]
    )
    ...
    dual_loop.assert_metadata_only(
        payload,
        label=f"patch-proposal-controlled-follow-up-feedback-backlog-bridge:{case_id}",
    )
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-controlled-follow-up-feedback-backlog-") as tmp:
        ...
        proc = subprocess.run(
            [
                ...
                str(
                    ROOT
                    / "scripts"
                    / "patch_proposal_customer_feedback_controlled_follow_up_feedback_backlog_bridge.py"
                ),
                "--case",
                "all",
                "--output-dir",
        ...
        dual_loop.assert_metadata_only(stdout, label="patch-proposal-controlled-follow-up-feedback-backlog-bridge-cli")
        if stdout.get("case_ids") != list(bridge.CASE_IDS):
            raise RuntimeError("Controlled follow-up feedback backlog bridge CLI case order drifted")
        for case_id, expected_artifacts in cases.items():
            for filename, expected in expected_artifacts.items():
                ...
                if not path.is_file():
                    raise RuntimeError(f"Controlled follow-up feedback backlog bridge CLI missing {case_id}/{filename}")
                if load_json(path) != expected:
                    raise RuntimeError(f"Controlled follow-up feedback backlog bridge CLI output drifted: {case_id}/{filename}")
            signal_path = output_dir / case_id / "product-loop-backlog-signal.json"
            if expected_artifacts["patch-proposal-controlled-follow-up-feedback-backlog-bridge.json"]["status"] == "queued_for_product_loop":
                if not signal_path.is_file():
                    raise RuntimeError(f"Controlled follow-up feedback backlog bridge CLI missing signal: {case_id}")
            elif signal_path.exists():
                raise RuntimeError(f"Controlled follow-up feedback backlog bridge CLI created signal for blocked case: {case_id}")
...
def negative_fixtures(pass_bridge: Mapping[str, Any]) -> dict[str, str]:
    ...
    def expect_rejected(name: str, mutator: Any) -> None:
        ...
        mutator(payload)
        try:
            bridge.validate_controlled_follow_up_feedback_backlog_bridge(payload)
...
def build_report() -> dict[str, Any]:
    ...
    if report["case_reports"] != case_reports:
        raise RuntimeError("Controlled follow-up feedback backlog bridge case summary drifted")
    report["schema_files"] = [
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-customer-signal"]["patch-proposal-controlled-follow-up-feedback-backlog-bridge.json"]
    )
...
def check_fixtures(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    for case_id, artifacts in cases.items():
        ...
        for filename, payload in artifacts.items():
            ...
            if not path.is_file():
                raise SystemExit(f"Missing controlled follow-up feedback backlog bridge fixture: {path}")
            if load_json(path) != payload:
                raise SystemExit(
                    f"Controlled follow-up feedback backlog bridge fixture is out of date: {path}. "
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_backlog_bridge.py --write"
                )
        signal_path = case_dir / "product-loop-backlog-signal.json"
        if (
            artifacts["patch-proposal-controlled-follow-up-feedback-backlog-bridge.json"]["status"] == "blocked"
            and signal_path.exists()
        ):
            raise SystemExit(f"Blocked controlled follow-up feedback backlog case must not include signal: {signal_path}")
...
def markdown_report(report: Mapping[str, Any]) -> str:
    lines = [
        ...
        "",
        "Metadata-only proof that accepted controlled follow-up feedback intake receipts can emit Product Loop backlog signal refs without assigning priority or mutating a live backlog.",
        "",
...
def html_report(markdown: str) -> str:
    ...
    return (
        ...
        "<html lang=\"en\">\n"
        "<meta charset=\"utf-8\">\n"
        "<title>Patch Proposal Controlled Follow-up Feedback Backlog Bridge</title>\n"
        "<body><pre>"
        + escaped
        + "</pre></body></html>\n"
    )
...
def main() -> int:
    ...
    if args.write:
        ...
        print("wrote controlled follow-up feedback backlog bridge fixtures and reports")
        return 0
    ...
    if args.check:
        ...
        if not REPORT.is_file() or REPORT.read_text(encoding="utf-8") != report_text:
            raise SystemExit(
                "Controlled follow-up feedback backlog bridge report is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_backlog_bridge.py --write"
            )
        if not MARKDOWN_REPORT.is_file() or MARKDOWN_REPORT.read_text(encoding="utf-8") != md:
            raise SystemExit(
                "Controlled follow-up feedback backlog bridge markdown is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_backlog_bridge.py --write"
            )
        if not HTML_REPORT.is_file() or HTML_REPORT.read_text(encoding="utf-8") != html:
            raise SystemExit(
                "Controlled follow-up feedback backlog bridge HTML is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_backlog_bridge.py --write"
            )
        print("ok    controlled follow-up feedback backlog bridge report is up to date")
        return 0
