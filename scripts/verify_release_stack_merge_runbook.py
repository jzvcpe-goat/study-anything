...
def build_runbook(manifest: dict[str, Any], allow_missing_top_pr: bool, max_depth: int) -> dict[str, Any]:
    ...
    ordered_lineage = list(reversed(lineage))
        ...
        steps.append(
            {
                ...
                "retarget_after_previous_merge_command": (
                    f"gh pr edit {pr_number} --base {target_branch}" if base != target_branch else None
                ),
    ...
    ready_to_merge_now = status == "pass" and not draft_prs
    return {
