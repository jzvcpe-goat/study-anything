...
import patch_proposal_customer_feedback_controlled_follow_up_boundary_gate as boundary_gate  # noqa: E402


REHEARSAL_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-rehearsal-receipt-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-controlled-follow-up-rehearsal-v1"
...
SOURCE_FIXTURE_DIR = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-boundary-gate"
...
PRIVACY_FLAGS = {
    **boundary_gate.PRIVACY_FLAGS,
    "controlled_follow_up_rehearsal_metadata_only": True,
    "follow_up_rehearsal_receipt_only": True,
...
def source_envelope_refs(case_id: str) -> dict[str, Any] | None:
    ...
    if flags["source_missing"]:
        return None
    path = SOURCE_FIXTURE_DIR / str(flags["source_case"]) / "patch-proposal-controlled-follow-up-envelope-refs.json"
    refs = load_json(path)
...
def _source_ref(case_id: str, refs: Mapping[str, Any] | None) -> dict[str, Any] | None:
    ...
    return {
        "kind": "patch-proposal-controlled-follow-up-envelope-refs",
        "path": (
            "fixtures/patch-proposal-customer-feedback-controlled-follow-up-boundary-gate/"
            f"{source_case}/patch-proposal-controlled-follow-up-envelope-refs.json"
        ),
        "case_id": source_case,
...
def build_controlled_follow_up_rehearsal(case_id: str) -> dict[str, Any]:
    ...
    receipt = {
        **_base(REHEARSAL_SCHEMA_VERSION),
        "rehearsal_id": f"patch-proposal-controlled-follow-up-rehearsal-{case_id}",
        "case_id": case_id,
        "status": "ready" if ready else "blocked",
        "decision": "ready_for_local_follow_up_rehearsal_review" if ready else "block_controlled_follow_up_rehearsal",
        ...
        "rehearsal_summary": {
            "package_type": "metadata_only_patch_proposal_controlled_follow_up_rehearsal",
        ...
        "effect_boundary": {
            "follow_up_rehearsal_receipt_written": True,
            "follow_up_preview_body_included": False,
...
def validate_controlled_follow_up_rehearsal(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if summary.get("package_type") != "metadata_only_patch_proposal_controlled_follow_up_rehearsal":
...
def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    return {
        "patch-proposal-controlled-follow-up-rehearsal-receipt.json": build_controlled_follow_up_rehearsal(
            case_id
        )
    }
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_controlled_follow_up_rehearsal(
            cases[case_id]["patch-proposal-controlled-follow-up-rehearsal-receipt.json"]
        )
        case_reports.append(
            {
                "case_id": case_id,
        ...
        **_base(REPORT_SCHEMA_VERSION),
        ...
        "source_chain": {
            ...
            "ready_envelope_refs": [
                (
                    "fixtures/patch-proposal-customer-feedback-controlled-follow-up-boundary-gate/"
                    f"{case_id}/patch-proposal-controlled-follow-up-envelope-refs.json"
                )
