#!/usr/bin/env python3
"""Verify controlled follow-up feedback Product Owner gate artifacts."""
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_product_owner_gate as gate  # noqa: E402


FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-owner-gate"
REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-product-owner-gate.json"
)
MARKDOWN_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-product-owner-gate.md"
)
HTML_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-product-owner-gate.html"
)
REPORT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-owner-gate-v1.schema.json"
)
RECEIPT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-controlled-follow-up-feedback-product-owner-receipt-v1.schema.json"
)
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    receipt = gate.validate_product_owner_receipt(
        artifacts["patch-proposal-controlled-follow-up-feedback-product-owner-receipt.json"]
    )
    ...
    dual_loop.assert_metadata_only(
        receipt,
        label=f"patch-proposal-controlled-follow-up-feedback-product-owner-gate:{case_id}",
    )
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-controlled-follow-up-feedback-owner-") as tmp:
        ...
        proc = subprocess.run(
            [
                ...
                str(
                    ROOT
                    / "scripts"
                    / "patch_proposal_customer_feedback_controlled_follow_up_feedback_product_owner_gate.py"
                ),
                "--case",
                "all",
                "--output-dir",
        ...
        dual_loop.assert_metadata_only(stdout, label="patch-proposal-controlled-follow-up-feedback-product-owner-gate-cli")
        if stdout.get("case_ids") != list(gate.CASE_IDS):
            raise RuntimeError("Controlled follow-up Product Owner gate CLI case order drifted")
        for case_id, expected_artifacts in cases.items():
            for filename, expected in expected_artifacts.items():
                ...
                if not path.is_file():
                    raise RuntimeError(f"Controlled follow-up Product Owner gate CLI missing {case_id}/{filename}")
                if load_json(path) != expected:
                    raise RuntimeError(f"Controlled follow-up Product Owner gate CLI output drifted: {case_id}/{filename}")
            candidate_path = output_dir / case_id / "patch-proposal-product-spec-eval-candidate.json"
            if (
                expected_artifacts["patch-proposal-controlled-follow-up-feedback-product-owner-receipt.json"]["status"]
                == "queued_for_spec_eval_candidate"
            ):
                if not candidate_path.is_file():
                    raise RuntimeError(f"Controlled follow-up Product Owner gate CLI missing candidate: {case_id}")
            elif candidate_path.exists():
                raise RuntimeError(f"Controlled follow-up Product Owner gate CLI created candidate for blocked case: {case_id}")
...
def build_report() -> dict[str, Any]:
    ...
    if report["case_reports"] != case_reports:
        raise RuntimeError("Controlled follow-up Product Owner gate case summary drifted")
    report["schema_files"] = [
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-customer-signal"]["patch-proposal-controlled-follow-up-feedback-product-owner-receipt.json"]
    )
...
def check_fixtures(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    for case_id, artifacts in cases.items():
        ...
        for filename, payload in artifacts.items():
            ...
            if not path.is_file():
                raise SystemExit(f"Missing controlled follow-up Product Owner gate fixture: {path}")
            if load_json(path) != payload:
                raise SystemExit(
                    f"Controlled follow-up Product Owner gate fixture is out of date: {path}. "
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_product_owner_gate.py --write"
                )
        candidate_path = case_dir / "patch-proposal-product-spec-eval-candidate.json"
        if (
            artifacts["patch-proposal-controlled-follow-up-feedback-product-owner-receipt.json"]["status"] == "blocked"
            and candidate_path.exists()
        ):
            raise SystemExit(f"Blocked controlled follow-up Product Owner gate case must not include candidate: {candidate_path}")
...
def html_report(markdown: str) -> str:
    ...
    return (
        ...
        "<html lang=\"en\">\n"
        "<meta charset=\"utf-8\">\n"
        "<title>Patch Proposal Controlled Follow-up Feedback Product Owner Gate</title>\n"
        "<body><pre>"
        + escaped
        + "</pre></body></html>\n"
    )
...
def main() -> int:
    ...
    if args.write:
        ...
        print("wrote controlled follow-up Product Owner gate fixtures and reports")
        return 0
    ...
    if args.check:
        ...
        if not REPORT.is_file() or REPORT.read_text(encoding="utf-8") != report_text:
            raise SystemExit(
                "Controlled follow-up Product Owner gate report is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_product_owner_gate.py --write"
            )
        if not MARKDOWN_REPORT.is_file() or MARKDOWN_REPORT.read_text(encoding="utf-8") != md:
            raise SystemExit(
                "Controlled follow-up Product Owner gate markdown is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_product_owner_gate.py --write"
            )
        if not HTML_REPORT.is_file() or HTML_REPORT.read_text(encoding="utf-8") != html:
            raise SystemExit(
                "Controlled follow-up Product Owner gate HTML is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_product_owner_gate.py --write"
            )
        print("ok    controlled follow-up Product Owner gate report is up to date")
        return 0
