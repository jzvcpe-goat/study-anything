#!/usr/bin/env python3
"""Verify Patch Proposal Customer Feedback Controlled Follow-up Outcome Receipt artifacts."""
...
FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-outcome"
REPORT = (
    ROOT
    / "platform"
    / "generated"
...
MARKDOWN_REPORT = (
    ...
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-outcome.md"
)
HTML_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-outcome.html"
)
REPORT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-customer-feedback-controlled-follow-up-outcome-v1.schema.json"
)
RECEIPT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-controlled-follow-up-outcome-receipt-v1.schema.json"
)


EXPECTED = {
    "pass-human-operator": ("recorded", "record_external_controlled_follow_up_outcome", []),
    "pass-host-platform-agent": ("recorded", "record_external_controlled_follow_up_outcome", []),
    "blocked-rehearsal-blocked": (
        "blocked",
        "block_controlled_follow_up_outcome",
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    receipt = outcome.validate_controlled_follow_up_outcome(
        artifacts["patch-proposal-controlled-follow-up-outcome-receipt.json"]
    )
    ...
    dual_loop.assert_metadata_only(receipt, label=f"patch-proposal-controlled-follow-up-outcome:{case_id}")
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-controlled-follow-up-outcome-") as tmp:
        ...
        proc = subprocess.run(
            [
                ...
                str(
                    ROOT
                    / "scripts"
                    / "patch_proposal_customer_feedback_controlled_follow_up_outcome_receipt.py"
                ),
                "--case",
                "all",
                "--output-dir",
        ...
        dual_loop.assert_metadata_only(stdout, label="patch-proposal-controlled-follow-up-outcome-cli")
        if stdout.get("case_ids") != list(outcome.CASE_IDS):
            raise RuntimeError("Patch Proposal Controlled Follow-up Outcome CLI case order drifted")
        for case_id, expected_artifacts in cases.items():
            for filename, expected in expected_artifacts.items():
                ...
                if not path.is_file():
                    raise RuntimeError(f"Patch Proposal Controlled Follow-up Outcome CLI missing {case_id}/{filename}")
                if load_json(path) != expected:
                    raise RuntimeError(
                        f"Patch Proposal Controlled Follow-up Outcome CLI output drifted: {case_id}/{filename}"
                    )
...
def negative_fixtures(pass_outcome: Mapping[str, Any]) -> dict[str, str]:
    ...
    def expect_rejected(name: str, mutator: Any) -> None:
        ...
        mutator(payload)
        try:
            outcome.validate_controlled_follow_up_outcome(payload)
...
def build_report() -> dict[str, Any]:
    ...
    if report["case_reports"] != case_reports:
        raise RuntimeError("Patch Proposal Controlled Follow-up Outcome report case summary drifted")
    report["schema_files"] = [
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-human-operator"]["patch-proposal-controlled-follow-up-outcome-receipt.json"]
    )
...
def check_fixtures(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    for case_id, artifacts in cases.items():
        ...
        for filename, payload in artifacts.items():
            ...
            if not path.is_file():
                raise SystemExit(f"Missing patch proposal controlled follow-up outcome fixture: {path}")
            if load_json(path) != payload:
                raise SystemExit(
                    f"Patch proposal controlled follow-up outcome fixture is out of date: {path}. "
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_outcome_receipt.py --write"
                )
        extra = sorted(path.name for path in case_dir.glob("*.json") if path.name not in expected_names)
        if extra:
            raise SystemExit(f"Unexpected patch proposal controlled follow-up outcome fixture(s): {case_dir}: {extra}")
...
def html_report(markdown: str) -> str:
    ...
    return (
        ...
        "<html lang=\"en\">\n"
        "<meta charset=\"utf-8\">\n"
        "<title>Patch Proposal Controlled Follow-up Outcome Receipt</title>\n"
        "<body><pre>"
        + escaped
        + "</pre></body></html>\n"
    )
...
def main() -> int:
    ...
    if args.write:
        ...
        print("wrote patch proposal controlled follow-up outcome fixtures and reports")
        return 0
    ...
    if args.check:
        ...
        if not REPORT.is_file() or REPORT.read_text(encoding="utf-8") != report_text:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Outcome report is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_outcome_receipt.py --write"
            )
        if not MARKDOWN_REPORT.is_file() or MARKDOWN_REPORT.read_text(encoding="utf-8") != md:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Outcome markdown is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_outcome_receipt.py --write"
            )
        if not HTML_REPORT.is_file() or HTML_REPORT.read_text(encoding="utf-8") != html:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Outcome HTML is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_outcome_receipt.py --write"
            )
        print("ok    Patch Proposal Controlled Follow-up Outcome report is up to date")
        return 0
