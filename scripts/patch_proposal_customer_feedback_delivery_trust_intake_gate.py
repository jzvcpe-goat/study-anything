#!/usr/bin/env python3
"""Gate Patch Proposal Product Loop runs into Delivery Trust case candidates.

This layer consumes Product Loop scenario/run candidates emitted by the Patch
Proposal Product Loop Brief Intake Gate. It requires controlled-failure and
attention-reconstruction evidence, then emits only a metadata-only Delivery
Trust case candidate.
...
import patch_proposal_customer_feedback_product_loop_brief_intake_gate as brief_intake_gate  # noqa: E402


RECEIPT_SCHEMA_VERSION = "patch-proposal-customer-feedback-delivery-trust-intake-receipt-v1"
CANDIDATE_SCHEMA_VERSION = "patch-proposal-delivery-trust-case-candidate-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-delivery-trust-intake-gate-v1"

DEFAULT_OUTPUT_DIR = ROOT / ".cognitive-loop" / "artifacts" / "patch-proposal-customer-feedback-delivery-trust-intake-gate"
...
CLAIM_BOUNDARY = {
    "current_claim": (
        ...
        "Trust case candidate only when Product Loop, controlled-failure, "
        "attention-reconstruction, and Dual Loop gate evidence are all present. "
        "The candidate stops before Delivery Trust Case Harness invocation."
    ),
    "not_claimed": [
...
def build_candidate(
    ...
    candidate = {
        **_base(CANDIDATE_SCHEMA_VERSION),
        "candidate_id": f"patch-proposal-delivery-trust-case-candidate-{run_hash[:16]}",
    ...
    *,
    ...
    receipt = {
        **_base(RECEIPT_SCHEMA_VERSION),
        "intake_receipt_id": (
            f"patch-proposal-delivery-trust-intake-{run_hash[:16]}"
            if run_hash
            else f"patch-proposal-delivery-trust-intake-blocked-{case_id}"
        ),
        "case_id": case_id,
        ...
        "decision": (
            "create_patch_proposal_delivery_trust_case_candidate"
            if candidate
            else "block_patch_proposal_delivery_trust_intake_gate"
        ),
        "blocked_reasons": reasons,
        "source_refs": {
            "product_loop_run_ref": (
                f"fixtures/patch-proposal-customer-feedback-product-loop-brief-intake-gate/"
...
def validate_intake_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if status == "queued_for_delivery_trust_case_candidate":
        if decision != "create_patch_proposal_delivery_trust_case_candidate":
        ...
        if decision != "block_patch_proposal_delivery_trust_intake_gate":
...
def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    ...
    artifacts = {"patch-proposal-delivery-trust-intake-receipt.json": receipt}
    candidate = receipt.get("candidate")
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_intake_receipt(cases[case_id]["patch-proposal-delivery-trust-intake-receipt.json"])
        case_reports.append(
            {
                "case_id": case_id,
...
def main() -> int:
    ...
    if args.product_loop_run:
        ...
        selected = {
            "custom": {
                "patch-proposal-delivery-trust-intake-receipt.json": build_intake_receipt(
                    "custom",
        ...
        dump_json(
            {
                ...
                "queued_case_count": sum(
                    ...
                    if artifacts["patch-proposal-delivery-trust-intake-receipt.json"]["status"]
                    == "queued_for_delivery_trust_case_candidate"
                ),
                "blocked_case_count": sum(
                    1
                    for artifacts in selected.values()
                    if artifacts["patch-proposal-delivery-trust-intake-receipt.json"]["status"] == "blocked"
                ),
                "model_calls_performed": False,
