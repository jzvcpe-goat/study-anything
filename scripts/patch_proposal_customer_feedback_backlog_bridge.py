#!/usr/bin/env python3
"""Bridge Patch Proposal Customer Feedback Intake receipts into backlog signals.

The bridge is metadata-only. It can queue a Product Loop backlog signal from an
accepted feedback-intake receipt, but it cannot assign priority, send follow-up,
mutate source, mutate production, call models, or store raw customer material.
"""
...
import patch_proposal_customer_feedback_intake_receipt as feedback_intake  # noqa: E402


BRIDGE_SCHEMA_VERSION = "patch-proposal-customer-feedback-backlog-bridge-v1"
BACKLOG_SIGNAL_SCHEMA_VERSION = "product-loop-backlog-signal-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-backlog-bridge-v1"

DEFAULT_OUTPUT_DIR = ROOT / ".cognitive-loop" / "artifacts" / "patch-proposal-customer-feedback-backlog-bridge"
INTAKE_FIXTURE_DIR = ROOT / "fixtures" / "patch-proposal-customer-feedback-intake"
...
CLAIM_BOUNDARY = {
    "current_claim": (
        "An accepted patch-proposal customer-feedback intake receipt can become a "
        "metadata-only Product Loop backlog signal. The signal is not prioritized, "
        "not sent to a customer, not executed, and not promoted to production."
    ),
    "not_claimed": [
...
class PatchProposalCustomerFeedbackBacklogBridgeError(ValueError):
    """Raised when backlog bridge artifacts are unsafe or invalid."""
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise PatchProposalCustomerFeedbackBacklogBridgeError(f"Expected JSON object: {path}")
    return payload
...
def _require_object(payload: Mapping[str, Any], key: str, *, label: str) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise PatchProposalCustomerFeedbackBacklogBridgeError(f"{label}.{key} must be an object")
    return value
...
def _validate_privacy(payload: Mapping[str, Any], *, label: str) -> None:
    ...
    if forbidden:
        raise PatchProposalCustomerFeedbackBacklogBridgeError(f"{label} includes forbidden raw fields: {forbidden}")
    privacy = _require_object(payload, "privacy", label=label)
    for key, expected in PRIVACY_FLAGS.items():
        if privacy.get(key) is not expected:
            raise PatchProposalCustomerFeedbackBacklogBridgeError(f"{label}.privacy.{key} must be {expected!r}")
    ...
    for key, expected in ISOLATION.items():
        if isolation.get(key) is not expected:
            raise PatchProposalCustomerFeedbackBacklogBridgeError(f"{label}.isolation.{key} must be {expected!r}")


def _case_flags(case_id: str) -> dict[str, bool | str]:
    if case_id not in CASE_IDS:
        raise PatchProposalCustomerFeedbackBacklogBridgeError(f"Unknown case id: {case_id}")
    return {
        "intake_case": "blocked-raw-customer-reply" if case_id == "blocked-intake-blocked" else (
            "pass-operator-signal" if case_id == "pass-operator-signal" else (
                "pass-host-platform-agent-signal" if case_id == "pass-host-platform-agent-signal" else "pass-customer-signal"
            )
        ),
...
def source_intake(case_id: str) -> dict[str, Any]:
    ...
    receipt = load_json(INTAKE_FIXTURE_DIR / intake_case / "patch-proposal-customer-feedback-intake-receipt.json")
    return feedback_intake.validate_customer_feedback_intake(receipt)
...
def build_backlog_signal(case_id: str, receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    signal = {
        **_base(BACKLOG_SIGNAL_SCHEMA_VERSION),
        "signal_id": f"patch-proposal-product-loop-backlog-{source_hash[:16]}",
        ...
        "source_feedback_intake_ref": (
            f"fixtures/patch-proposal-customer-feedback-intake/{receipt['case_id']}/"
            "patch-proposal-customer-feedback-intake-receipt.json"
        ),
        "source_delivery_outcome_ref": source_refs["delivery_outcome_receipt_ref"],
        "source_delivery_outcome_hash": source_refs["delivery_outcome_receipt_hash"],
        "feedback_ref": {
...
def build_customer_feedback_backlog_bridge(case_id: str) -> dict[str, Any]:
    ...
    bridge = {
        **_base(BRIDGE_SCHEMA_VERSION),
        "bridge_id": f"patch-proposal-customer-feedback-backlog-{artifact_hash(receipt)[:16]}-{case_id}",
        "case_id": case_id,
        ...
        "source_refs": {
            "customer_feedback_intake_receipt_ref": (
                f"fixtures/patch-proposal-customer-feedback-intake/{receipt['case_id']}/"
                "patch-proposal-customer-feedback-intake-receipt.json"
            ),
            "customer_feedback_intake_receipt_hash": artifact_hash(receipt),
            "customer_feedback_intake_report_ref": (
        ...
        "checks": checks,
        "quality_gates": {
            "accepted_feedback_intake_required": True,
    ...
    return validate_customer_feedback_backlog_bridge(bridge)
...
def validate_backlog_signal(signal: Mapping[str, Any]) -> dict[str, Any]:
    if signal.get("schema_version") != BACKLOG_SIGNAL_SCHEMA_VERSION:
        raise PatchProposalCustomerFeedbackBacklogBridgeError("backlog signal schema_version drifted")
    _validate_privacy(signal, label=BACKLOG_SIGNAL_SCHEMA_VERSION)
    if signal.get("destination") != "product_loop_backlog":
        raise PatchProposalCustomerFeedbackBacklogBridgeError("backlog signal destination must be product_loop_backlog")
    if signal.get("next_boundary") != "product_owner_prioritization":
        raise PatchProposalCustomerFeedbackBacklogBridgeError("backlog signal must stop at product owner prioritization")
    if signal.get("priority_assignment") != "not_assigned":
        raise PatchProposalCustomerFeedbackBacklogBridgeError("backlog signal must not assign priority")
    if signal.get("requires_product_owner_prioritization") is not True:
        raise PatchProposalCustomerFeedbackBacklogBridgeError("backlog signal must require product owner prioritization")
    for key in ("ready_for_execution", "ready_for_customer_delivery"):
        if signal.get(key) is not False:
            raise PatchProposalCustomerFeedbackBacklogBridgeError(f"backlog signal {key} must remain false")
    blocked = signal.get("blocked_destinations")
    if not isinstance(blocked, list):
        raise PatchProposalCustomerFeedbackBacklogBridgeError("backlog signal blocked_destinations must be a list")
    for destination in (
        "automatic_priority_assignment",
        "automatic_follow_up",
        "customer_visible_action",
        ...
        if destination not in blocked:
            raise PatchProposalCustomerFeedbackBacklogBridgeError(f"backlog signal missing blocked destination: {destination}")
    ...
    for key, expected in EFFECT_BOUNDARY.items():
        if effect.get(key) is not expected:
            raise PatchProposalCustomerFeedbackBacklogBridgeError(f"backlog signal effect_boundary.{key} must be {expected!r}")
    return dict(signal)


def validate_customer_feedback_backlog_bridge(bridge: Mapping[str, Any]) -> dict[str, Any]:
    if bridge.get("schema_version") != BRIDGE_SCHEMA_VERSION:
        raise PatchProposalCustomerFeedbackBacklogBridgeError("customer feedback backlog bridge schema_version drifted")
    ...
    if status not in {"queued_for_product_loop", "blocked"}:
        raise PatchProposalCustomerFeedbackBacklogBridgeError("bridge status must be queued_for_product_loop or blocked")
    if not isinstance(blocked_reasons, list):
        raise PatchProposalCustomerFeedbackBacklogBridgeError("bridge blocked_reasons must be a list")
    effect = _require_object(bridge, "effect_boundary", label=BRIDGE_SCHEMA_VERSION)
    for key, expected in EFFECT_BOUNDARY.items():
        if effect.get(key) is not expected:
            raise PatchProposalCustomerFeedbackBacklogBridgeError(f"bridge effect_boundary.{key} must be {expected!r}")
    if status == "queued_for_product_loop":
        if decision != "emit_product_loop_backlog_signal":
            raise PatchProposalCustomerFeedbackBacklogBridgeError("queued bridge must emit backlog signal")
        if blocked_reasons:
            raise PatchProposalCustomerFeedbackBacklogBridgeError("queued bridge must not carry blocked reasons")
        if not isinstance(backlog_signal, Mapping):
            raise PatchProposalCustomerFeedbackBacklogBridgeError("queued bridge must include backlog signal")
        validate_backlog_signal(backlog_signal)
    else:
        if decision != "block_product_loop_backlog_signal":
            raise PatchProposalCustomerFeedbackBacklogBridgeError("blocked bridge must block backlog signal")
        if not blocked_reasons:
            raise PatchProposalCustomerFeedbackBacklogBridgeError("blocked bridge must include reasons")
        if backlog_signal is not None:
            raise PatchProposalCustomerFeedbackBacklogBridgeError("blocked bridge must not include backlog signal")
    return dict(bridge)


def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    bridge = build_customer_feedback_backlog_bridge(case_id)
    artifacts = {"patch-proposal-customer-feedback-backlog-bridge.json": bridge}
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        bridge = validate_customer_feedback_backlog_bridge(
            cases[case_id]["patch-proposal-customer-feedback-backlog-bridge.json"]
        )
        case_reports.append(
            {
                "case_id": case_id,
    ...
    report = {
        **_base(REPORT_SCHEMA_VERSION),
        "status": "pass",
        "purpose": (
            "Prove accepted Patch Proposal Customer Feedback Intake receipts can create metadata-only Product Loop "
            "backlog signals without raw replies, private customer data, automatic priority assignment, follow-up "
            "sending, source mutation, production mutation, secrets, or model credentials."
        ),
        "source_chain": {
            "customer_feedback_intake_report": (
                "platform/generated/study-anything-patch-proposal-customer-feedback-intake.json"
            ),
            "recorded_customer_feedback_intake_receipt": (
                "fixtures/patch-proposal-customer-feedback-intake/pass-customer-signal/"
                "patch-proposal-customer-feedback-intake-receipt.json"
            ),
        },
        "backlog_matrix": {
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "patch-proposal-customer-feedback-backlog-bridge-cli-result-v1",
        "status": "ok",
        ...
        "queued_backlog_signals": sum(
            ...
            if artifacts["patch-proposal-customer-feedback-backlog-bridge.json"]["status"] == "queued_for_product_loop"
        ),
        "blocked_backlog_signals": sum(
            1
            for artifacts in cases.values()
            if artifacts["patch-proposal-customer-feedback-backlog-bridge.json"]["status"] == "blocked"
        ),
        "privacy": dict(PRIVACY_FLAGS),
        "effect_boundary": dict(EFFECT_BOUNDARY),
    }
