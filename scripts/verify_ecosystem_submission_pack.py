...
COGNITIVE_LOOP_PR_CI_RECEIPT_SCHEMA_PATH = (
    ROOT / "platform" / "schemas" / "cognitive-loop-pr-ci-receipt.schema.json"
)
COGNITIVE_LOOP_PR_CI_SOURCE_SCHEMA_PATH = (
    ROOT / "platform" / "schemas" / "cognitive-loop-pr-ci-source.schema.json"
)
...
def verify_platform_submissions(by_id: dict[str, Any]) -> None:
    for platform_id, submission in sorted(by_id.items()):
        ...
        if "platform/schemas/cognitive-loop-pr-ci-receipt.schema.json" not in set(str(asset) for asset in import_assets):
            raise EcosystemSubmissionError(f"{platform_id} must include the Cognitive Loop PR CI receipt schema.")
        if "platform/schemas/cognitive-loop-pr-ci-source.schema.json" not in set(str(asset) for asset in import_assets):
            raise EcosystemSubmissionError(f"{platform_id} must include the Cognitive Loop PR CI source schema.")
...
def verify_cognitive_loop_apply_plan_report() -> None:
    report = load_json(COGNITIVE_LOOP_APPLY_PLAN_PATH)
    if report.get("schema_version") != "cognitive-loop-apply-plan-verification-v1":
        raise EcosystemSubmissionError("Cognitive Loop apply-plan verifier schema drifted.")
    if report.get("status") != "pass":
        raise EcosystemSubmissionError("Cognitive Loop apply-plan verifier must pass.")
    if report.get("artifact_schema") != "cognitive-loop-apply-plan-lite-v1":
        raise EcosystemSubmissionError("Cognitive Loop apply-plan artifact schema drifted.")
    if report.get("receipt_schema") != "cognitive-loop-apply-receipt-lite-v1":
        raise EcosystemSubmissionError("Cognitive Loop apply receipt schema drifted.")
    success = report.get("success_modes") or {}
    for key in (
        "receipt_written",
        "marker_written",
        "input_hash_unchanged",
        "apply_requires_allow_flag",
        "idempotent_receipt",
        "html_json_created",
    ):
        if success.get(key) is not True:
            raise EcosystemSubmissionError(f"Apply Plan success mode {key} must be true.")
    if success.get("dry_run_status") != "dry_run_ready":
        raise EcosystemSubmissionError("Apply Plan dry-run status drifted.")
    if success.get("apply_status") != "applied":
        raise EcosystemSubmissionError("Apply Plan explicit apply status drifted.")
    if success.get("eligible_action_count", 0) < 1:
        raise EcosystemSubmissionError("Apply Plan must include at least one eligible low-risk action.")
    failures = report.get("failure_modes") or {}
    for key in (
        "high_risk_manual_only",
        "high_risk_apply_rejected",
        "forbidden_path_apply_rejected",
        "secret_proposal_rejected",
        "raw_diff_proposal_rejected",
        "policy_weakening_rejected",
    ):
        if failures.get(key) is not True:
            raise EcosystemSubmissionError(f"Apply Plan failure mode {key} must be true.")
    privacy = report.get("privacy") or {}
    for key in (
        "source_text_included",
        "raw_diff_included",
...
def verify_cognitive_loop_patch_proposal_report() -> None:
    report = load_json(COGNITIVE_LOOP_PATCH_PROPOSAL_PATH)
    if report.get("schema_version") != "cognitive-loop-patch-proposal-verification-v1":
        raise EcosystemSubmissionError("Cognitive Loop patch-proposal verifier schema drifted.")
    if report.get("status") != "pass":
        raise EcosystemSubmissionError("Cognitive Loop patch-proposal verifier must pass.")
    if report.get("artifact_schema") != "cognitive-loop-patch-proposal-lite-v1":
        raise EcosystemSubmissionError("Cognitive Loop patch-proposal artifact schema drifted.")
    success = report.get("success_modes") or {}
    if success.get("low_risk_status") != "ready":
        raise EcosystemSubmissionError("Patch Proposal low-risk status drifted.")
    if success.get("mixed_status") != "needs_review":
        raise EcosystemSubmissionError("Patch Proposal mixed status drifted.")
    if success.get("category_count") != 6:
        raise EcosystemSubmissionError("Patch Proposal must cover six patch categories.")
    for key in ("html_json_created", "read_only"):
        if success.get(key) is not True:
            raise EcosystemSubmissionError(f"Patch Proposal success mode {key} must be true.")
    manual = report.get("manual_modes") or {}
    for key in (
        "high_risk_manual_only",
        "human_gate_manual_only",
        "forbidden_path_manual_only",
        "insufficient_comparison_degraded",
    ):
        if manual.get(key) is not True:
            raise EcosystemSubmissionError(f"Patch Proposal manual mode {key} must be true.")
    failures = report.get("failure_modes") or {}
    for key in (
        "secret_proposal_rejected",
        "raw_diff_proposal_rejected",
        ...
        if failures.get(key) is not True:
            raise EcosystemSubmissionError(f"Patch Proposal failure mode {key} must be true.")
    privacy = report.get("privacy") or {}
    if privacy.get("read_only") is not True:
        raise EcosystemSubmissionError("Patch Proposal privacy.read_only must be true.")
    for key in (
        "source_text_included",
        "raw_diff_included",
        "learner_answers_included",
...
def verify_cognitive_loop_recipe_cli_schemas_report() -> None:
    ...
    expected_reports = {
        ...
        (
            "cognitive_loop_pr_ci_source",
            "<synthetic:fixture:cognitive-loop-pr-ci-source-v1>",
            "cognitive-loop-pr-ci-source-v1",
        ),
        (
            "cognitive_loop_pr_ci_source",
            "<synthetic:gh_live:cognitive-loop-pr-ci-source-v1>",
            "cognitive-loop-pr-ci-source-v1",
        ),
    }
    actual_reports = {
    ...
    schema_files = set(distribution.get("schema_files", []))
    ...
    if schema_files != expected_schema_files:
        raise EcosystemSubmissionError("Cognitive Loop recipe CLI schema file distribution drifted.")
    if not COGNITIVE_LOOP_PR_CI_RECEIPT_SCHEMA_PATH.is_file():
        raise EcosystemSubmissionError("Cognitive Loop PR CI receipt schema file is missing.")
    if not COGNITIVE_LOOP_PR_CI_SOURCE_SCHEMA_PATH.is_file():
        raise EcosystemSubmissionError("Cognitive Loop PR CI source schema file is missing.")
    privacy = report.get("privacy") or {}
    for key in (
        "raw_source_text_included",
        "diff_bodies_included",
...
def verify_cognitive_loop_schema_pack_consumer_report() -> None:
    ...
    expected_files = {
        ...
        "study-anything-platform-adoption-pack/platform/schemas/cognitive-loop-pr-ci-receipt.schema.json",
        "study-anything-platform-adoption-pack/platform/schemas/cognitive-loop-pr-ci-source.schema.json",
    }
    ...
    if set(schema_bundle.get("schema_keys", [])) != {
        ...
        raise EcosystemSubmissionError("Cognitive Loop schema pack consumer schema keys drifted.")
    ...
    standalone_schema_files = report.get("standalone_schema_files")
    if not isinstance(standalone_schema_files, list) or len(standalone_schema_files) != 2:
        raise EcosystemSubmissionError("Cognitive Loop schema pack consumer standalone schema files drifted.")
