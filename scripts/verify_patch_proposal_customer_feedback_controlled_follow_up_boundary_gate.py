#!/usr/bin/env python3
"""Verify Patch Proposal Customer Feedback Controlled Follow-up Boundary Gate artifacts."""
...
import patch_proposal_customer_feedback_controlled_follow_up_boundary_gate as gate  # noqa: E402


FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-boundary-gate"
REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-boundary-gate.json"
)
MARKDOWN_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-boundary-gate.md"
)
HTML_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-boundary-gate.html"
)
REPORT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-customer-feedback-controlled-follow-up-boundary-gate-v1.schema.json"
)
RECEIPT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-controlled-follow-up-boundary-receipt-v1.schema.json"
)
...
EXPECTED = {
    ...
    "blocked-automatic-customer-send": ("blocked", ["customer_visible_send_rejected"], False),
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    receipt = gate.validate_boundary_receipt(
        artifacts["patch-proposal-controlled-follow-up-boundary-receipt.json"]
    )
    ...
    if refs_created:
        refs = gate.validate_envelope_refs(
            artifacts["patch-proposal-controlled-follow-up-envelope-refs.json"]
        )
    ...
    dual_loop.assert_metadata_only(receipt, label=f"patch-proposal-controlled-follow-up-boundary:{case_id}")
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-controlled-follow-up-gate-") as tmp:
        ...
        proc = subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "patch_proposal_customer_feedback_controlled_follow_up_boundary_gate.py"),
                "--case",
                "all",
                "--output-dir",
                str(output_dir),
        ...
        dual_loop.assert_metadata_only(stdout, label="patch-proposal-controlled-follow-up-boundary-cli")
        if stdout.get("case_ids") != list(gate.CASE_IDS):
            raise RuntimeError("Patch Proposal Controlled Follow-up Boundary Gate CLI case order drifted")
        for case_id, expected_artifacts in cases.items():
            for filename, expected in expected_artifacts.items():
                ...
                if not path.is_file():
                    raise RuntimeError(f"Patch Proposal Controlled Follow-up Boundary Gate CLI missing {case_id}/{filename}")
                if load_json(path) != expected:
                    raise RuntimeError(
                        f"Patch Proposal Controlled Follow-up Boundary Gate CLI output drifted: {case_id}/{filename}"
                    )
            refs_path = output_dir / case_id / "patch-proposal-controlled-follow-up-envelope-refs.json"
            if expected_artifacts["patch-proposal-controlled-follow-up-boundary-receipt.json"]["status"] == "blocked":
                if refs_path.exists():
        ...
        if custom_stdout.get("case_ids") != ["custom"]:
            raise RuntimeError("Patch Proposal Controlled Follow-up Boundary Gate custom CLI output drifted")
...
def negative_fixtures(pass_receipt: Mapping[str, Any]) -> dict[str, str]:
    ...
    if missing:
        raise RuntimeError(f"Patch Proposal Controlled Follow-up Boundary Gate negative checks missing: {missing}")
    return failures
...
def build_report() -> dict[str, Any]:
    ...
    if report["case_reports"] != case_reports:
        raise RuntimeError("Patch Proposal Controlled Follow-up Boundary Gate case summary drifted")
    report["schema_files"] = [
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-customer-signal"]["patch-proposal-controlled-follow-up-boundary-receipt.json"]
    )
...
def check_fixtures(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    for case_id, artifacts in cases.items():
        ...
        if extra_files:
            raise SystemExit(
                f"Patch Proposal Controlled Follow-up Boundary Gate fixture has stale files: "
                f"{case_id}: {extra_files}"
            )
        for filename, payload in artifacts.items():
            path = case_dir / filename
            if not path.is_file():
                raise SystemExit(f"Missing Patch Proposal Controlled Follow-up Boundary Gate fixture: {path}")
            if load_json(path) != payload:
                raise SystemExit(
                    f"Patch Proposal Controlled Follow-up Boundary Gate fixture is out of date: {path}. "
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_boundary_gate.py --write"
                )
        refs_path = case_dir / "patch-proposal-controlled-follow-up-envelope-refs.json"
        if (
            artifacts["patch-proposal-controlled-follow-up-boundary-receipt.json"]["status"] == "blocked"
            and refs_path.exists()
        ):
            raise SystemExit(f"Blocked Patch Proposal Controlled Follow-up Boundary Gate case must not include refs: {case_id}")
...
def main() -> int:
    ...
    html = dual_loop.render_html_report("Patch Proposal Controlled Follow-up Boundary Gate", report)
    ...
    if args.write:
        ...
        print("wrote Patch Proposal Controlled Follow-up Boundary Gate fixtures and reports")
        return 0
    ...
    if args.check:
        ...
        if not REPORT.is_file() or REPORT.read_text(encoding="utf-8") != report_text:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Boundary Gate report is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_boundary_gate.py --write"
            )
        if not MARKDOWN_REPORT.is_file() or MARKDOWN_REPORT.read_text(encoding="utf-8") != md:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Boundary Gate markdown is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_boundary_gate.py --write"
            )
        if not HTML_REPORT.is_file() or HTML_REPORT.read_text(encoding="utf-8") != html:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Boundary Gate HTML is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_boundary_gate.py --write"
            )
        print("ok    Patch Proposal Controlled Follow-up Boundary Gate report is up to date")
        return 0
