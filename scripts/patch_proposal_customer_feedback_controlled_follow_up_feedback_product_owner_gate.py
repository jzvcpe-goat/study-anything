#!/usr/bin/env python3
"""Gate controlled follow-up feedback backlog signals before spec/eval work."""
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_backlog_bridge as backlog_bridge  # noqa: E402


RECEIPT_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-feedback-product-owner-receipt-v1"
CANDIDATE_SCHEMA_VERSION = "patch-proposal-product-spec-eval-candidate-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-owner-gate-v1"
...
CLAIM_BOUNDARY = {
    "current_claim": (
        "A controlled follow-up feedback backlog signal can enter the spec/eval "
...
class PatchProposalControlledFollowUpFeedbackProductOwnerGateError(ValueError):
    """Raised when the controlled follow-up Product Owner gate is unsafe."""
...
def source_bridge(case_id: str) -> dict[str, Any]:
    ...
    bridge = backlog_bridge.build_controlled_follow_up_feedback_backlog_bridge(bridge_case)
    return backlog_bridge.validate_controlled_follow_up_feedback_backlog_bridge(bridge)
...
def build_candidate(backlog_signal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    candidate = {
        **_base(CANDIDATE_SCHEMA_VERSION),
        "candidate_id": f"patch-proposal-controlled-follow-up-spec-eval-candidate-{signal_hash[:16]}",
        ...
        "source_controlled_follow_up_outcome_ref": signal["source_controlled_follow_up_outcome_ref"],
        "source_controlled_follow_up_outcome_hash": signal["source_controlled_follow_up_outcome_hash"],
...
def build_product_owner_receipt(
    ...
    *,
    ...
    bridge = (
        backlog_bridge.validate_controlled_follow_up_feedback_backlog_bridge(source_bridge_payload)
    ...
    receipt = {
        **_base(RECEIPT_SCHEMA_VERSION),
        ...
        "source_refs": {
            "backlog_bridge_ref": (
                "fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge/"
                f"{bridge_case}/patch-proposal-controlled-follow-up-feedback-backlog-bridge.json"
            ),
            ...
            "controlled_follow_up_feedback_intake_ref": (
                validated_signal.get("source_feedback_intake_ref") if validated_signal else None
            ),
            "controlled_follow_up_feedback_intake_hash": (
                validated_signal.get("source_feedback_intake_hash") if validated_signal else None
            ),
            "controlled_follow_up_outcome_ref": (
                validated_signal.get("source_controlled_follow_up_outcome_ref") if validated_signal else None
            ),
            "controlled_follow_up_outcome_hash": (
                validated_signal.get("source_controlled_follow_up_outcome_hash") if validated_signal else None
            ),
        },
...
def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    ...
    artifacts = {"patch-proposal-controlled-follow-up-feedback-product-owner-receipt.json": receipt}
    candidate = receipt.get("candidate")
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_product_owner_receipt(
            cases[case_id]["patch-proposal-controlled-follow-up-feedback-product-owner-receipt.json"]
        )
        case_reports.append(
            {
                "case_id": case_id,
    ...
    report = {
        **_base(REPORT_SCHEMA_VERSION),
        "status": "pass",
        "purpose": (
            "Prove controlled follow-up feedback backlog signals can create metadata-only spec/eval "
            "candidates only after active Product Owner boundary reconstruction, without priority assignment, "
            "execution, customer-visible follow-up, source mutation, production mutation, model calls, secrets, "
            "or model credentials."
        ),
        "source_chain": {
            "controlled_follow_up_feedback_backlog_bridge_report": (
                "platform/generated/"
                "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge.json"
            ),
            "recorded_backlog_signal": (
                "fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-backlog-bridge/"
                "pass-customer-signal/product-loop-backlog-signal.json"
            ),
        },
...
def main() -> int:
    ...
    result: dict[str, Any] = {
        "schema_version": "patch-proposal-controlled-follow-up-feedback-product-owner-gate-cli-result-v1",
        "status": "ok",
        "case_ids": list(cases),
        ...
        "queued_spec_eval_candidates": sum(
            ...
            if artifacts["patch-proposal-controlled-follow-up-feedback-product-owner-receipt.json"]["status"]
