#!/usr/bin/env python3
"""Gate controlled follow-up feedback Product Loop brief candidates into Product Loop runs.

This layer consumes `patch-proposal-product-loop-brief-candidate-v1` artifacts
from the controlled follow-up feedback Spec/Eval Authoring Gate. It can emit
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_spec_eval_authoring_gate as authoring_gate  # noqa: E402


RECEIPT_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-gate-v1"

DEFAULT_OUTPUT_DIR = ROOT / ".cognitive-loop" / "artifacts" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-gate"
...
CLAIM_BOUNDARY = {
    "current_claim": (
        "A Patch Proposal controlled follow-up feedback Product Loop brief candidate "
...
def source_authoring_receipt(case_id: str) -> dict[str, Any]:
    return source_authoring_artifacts(case_id)["patch-proposal-controlled-follow-up-feedback-spec-eval-authoring-receipt.json"]
...
def _brief_candidate_ref(candidate: Mapping[str, Any], candidate_hash: str) -> dict[str, Any]:
    return {
        ...
        "source_controlled_follow_up_outcome_ref": candidate.get("source_controlled_follow_up_outcome_ref"),
        "source_controlled_follow_up_outcome_hash": candidate.get("source_controlled_follow_up_outcome_hash"),
    ...
    *,
    ...
    receipt = {
        **_base(RECEIPT_SCHEMA_VERSION),
        "intake_receipt_id": (
            f"patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-{source_hash[:16]}"
            if source_hash
            else f"patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-blocked-{case_id}"
        ),
        "case_id": case_id,
        ...
        "source_refs": {
            "authoring_receipt_ref": (
                f"fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-spec-eval-authoring-gate/"
                f"{case_id if case_id in PASS_CASE_IDS else 'pass-customer-signal'}/"
                "patch-proposal-controlled-follow-up-feedback-spec-eval-authoring-receipt.json"
            ),
            "brief_candidate_ref": (
                f"fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-spec-eval-authoring-gate/"
                f"{case_id if case_id in PASS_CASE_IDS else 'pass-customer-signal'}/"
...
def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    ...
    artifacts = {"patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt.json": receipt}
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_intake_receipt(
            cases[case_id]["patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt.json"]
        )
        case_reports.append(
            {
                "case_id": case_id,
    ...
    report = {
        **_base(REPORT_SCHEMA_VERSION),
        "status": "pass",
        "purpose": (
            "Prove Patch Proposal controlled follow-up feedback Product Loop brief candidates can create "
...
def main() -> int:
    ...
    if args.brief_candidate:
        ...
        selected = {
            "custom": {
                "patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt.json": build_intake_receipt(
                    "custom",
    ...
    print(
        dump_json(
            {
                "schema_version": "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-cli-result-v1",
                "status": "ok",
