#!/usr/bin/env python3
"""Verify Patch Proposal Customer Feedback Controlled Follow-up Feedback Delivery Trust Intake Gate artifacts."""
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_intake_gate as gate  # noqa: E402


FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-intake-gate"
REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-intake-gate.json"
MARKDOWN_REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-intake-gate.md"
HTML_REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-intake-gate.html"
REPORT_SCHEMA_FILE = ROOT / "platform" / "schemas" / "cbb" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-delivery-trust-intake-gate-v1.schema.json"
RECEIPT_SCHEMA_FILE = ROOT / "platform" / "schemas" / "cbb" / "patch-proposal-controlled-follow-up-feedback-delivery-trust-intake-receipt-v1.schema.json"
CANDIDATE_SCHEMA_FILE = ROOT / "platform" / "schemas" / "cbb" / "patch-proposal-delivery-trust-case-candidate-v1.schema.json"


EXPECTED = {
    "pass-customer-signal": ("queued_for_delivery_trust_case_candidate", [], True),
    "pass-operator-signal": ("queued_for_delivery_trust_case_candidate", [], True),
    "pass-host-platform-agent-signal": ("queued_for_delivery_trust_case_candidate", [], True),
    "blocked-missing-product-loop-run": ("blocked", ["product_loop_run_missing"], False),
    "blocked-invalid-product-loop-run": ("blocked", ["product_loop_run_invalid"], False),
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    receipt = gate.validate_intake_receipt(artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-intake-receipt.json"])
    ...
    dual_loop.assert_metadata_only(receipt, label=f"patch-proposal-controlled-follow-up-feedback-delivery-trust-intake:{case_id}")
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-delivery-trust-intake-") as tmp:
        ...
        proc = subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_intake_gate.py"),
                "--case",
                "all",
                "--output-dir",
                str(output_dir),
        ...
        dual_loop.assert_metadata_only(stdout, label="patch-proposal-controlled-follow-up-feedback-delivery-trust-intake-cli")
        if stdout.get("case_ids") != list(gate.CASE_IDS):
            raise RuntimeError("Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake CLI case order drifted")
        for case_id, expected_artifacts in cases.items():
            for filename, expected in expected_artifacts.items():
                ...
                if not path.is_file():
                    raise RuntimeError(f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake CLI missing {case_id}/{filename}")
                if load_json(path) != expected:
                    raise RuntimeError(f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake CLI output drifted: {case_id}/{filename}")
            candidate_path = output_dir / case_id / "patch-proposal-delivery-trust-case-candidate.json"
            if expected_artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-intake-receipt.json"]["status"] == "blocked":
        ...
        if custom_stdout.get("case_ids") != ["custom"]:
            raise RuntimeError("Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake custom CLI output drifted")
...
def negative_fixtures(pass_receipt: Mapping[str, Any]) -> dict[str, str]:
    ...
    expect_rejected(
        ...
        lambda payload: (
            ...
            payload.__setitem__("decision", "block_patch_proposal_controlled_follow_up_feedback_delivery_trust_intake_gate"),
    ...
    if missing:
        raise RuntimeError(f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake negative checks missing: {missing}")
    return failures
...
def build_report() -> dict[str, Any]:
    ...
    if report["case_reports"] != case_reports:
        raise RuntimeError("Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake case summary drifted")
    report["schema_files"] = [
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-customer-signal"]["patch-proposal-controlled-follow-up-feedback-delivery-trust-intake-receipt.json"]
    )
...
def check_fixtures(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    for case_id, artifacts in cases.items():
        ...
        for filename, payload in artifacts.items():
            ...
            if not path.is_file():
                raise SystemExit(f"Missing Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake fixture: {path}")
            if load_json(path) != payload:
                raise SystemExit(
                    f"Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake fixture is out of date: {path}. "
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_intake_gate.py --write"
                )
        candidate_path = case_dir / "patch-proposal-delivery-trust-case-candidate.json"
        if (
            artifacts["patch-proposal-controlled-follow-up-feedback-delivery-trust-intake-receipt.json"]["status"] == "blocked"
            and candidate_path.exists()
        ):
            raise SystemExit(f"Blocked Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake case must not include candidate: {case_id}")
...
def main() -> int:
    ...
    html = dual_loop.render_html_report("Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake Gate", report)
    ...
    if args.write:
        ...
        print("wrote Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake fixtures and reports")
        return 0
    ...
    if args.check:
        ...
        if not REPORT.is_file() or REPORT.read_text(encoding="utf-8") != report_text:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake report is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_intake_gate.py --write"
            )
        if not MARKDOWN_REPORT.is_file() or MARKDOWN_REPORT.read_text(encoding="utf-8") != md:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake markdown is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_intake_gate.py --write"
            )
        if not HTML_REPORT.is_file() or HTML_REPORT.read_text(encoding="utf-8") != html:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake HTML is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_delivery_trust_intake_gate.py --write"
            )
        print("ok    Patch Proposal Controlled Follow-up Feedback Delivery Trust Intake report is up to date")
        return 0
