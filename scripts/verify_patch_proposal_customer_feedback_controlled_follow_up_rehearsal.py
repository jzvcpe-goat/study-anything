#!/usr/bin/env python3
"""Verify Patch Proposal Customer Feedback Controlled Follow-up Rehearsal artifacts."""
...
FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-rehearsal"
REPORT = (
    ROOT
    / "platform"
    / "generated"
...
MARKDOWN_REPORT = (
    ...
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-rehearsal.md"
)
HTML_REPORT = (
    ROOT
    / "platform"
    / "generated"
    / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-rehearsal.html"
)
REPORT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-customer-feedback-controlled-follow-up-rehearsal-v1.schema.json"
)
RECEIPT_SCHEMA_FILE = (
    ROOT
    / "platform"
    / "schemas"
    / "cbb"
    / "patch-proposal-controlled-follow-up-rehearsal-receipt-v1.schema.json"
)


EXPECTED = {
    "pass-customer-signal": ("ready", "ready_for_local_follow_up_rehearsal_review", []),
    "pass-operator-signal": ("ready", "ready_for_local_follow_up_rehearsal_review", []),
    "pass-host-platform-agent-signal": ("ready", "ready_for_local_follow_up_rehearsal_review", []),
    "blocked-missing-envelope-refs": (
        "blocked",
        "block_controlled_follow_up_rehearsal",
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    receipt = rehearsal.validate_controlled_follow_up_rehearsal(
        artifacts["patch-proposal-controlled-follow-up-rehearsal-receipt.json"]
    )
    ...
    dual_loop.assert_metadata_only(receipt, label=f"patch-proposal-controlled-follow-up-rehearsal:{case_id}")
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-controlled-follow-up-rehearsal-") as tmp:
        ...
        proc = subprocess.run(
            [
                ...
                str(
                    ROOT
                    / "scripts"
                    / "patch_proposal_customer_feedback_controlled_follow_up_rehearsal.py"
                ),
                "--case",
                "all",
                "--output-dir",
        ...
        dual_loop.assert_metadata_only(stdout, label="patch-proposal-controlled-follow-up-rehearsal-cli")
        if stdout.get("case_ids") != list(rehearsal.CASE_IDS):
            raise RuntimeError("Patch Proposal Controlled Follow-up Rehearsal CLI case order drifted")
        for case_id, expected_artifacts in cases.items():
            for filename, expected in expected_artifacts.items():
                ...
                if not path.is_file():
                    raise RuntimeError(f"Patch Proposal Controlled Follow-up Rehearsal CLI missing {case_id}/{filename}")
                if load_json(path) != expected:
                    raise RuntimeError(
                        f"Patch Proposal Controlled Follow-up Rehearsal CLI output drifted: {case_id}/{filename}"
                    )
...
def build_report() -> dict[str, Any]:
    ...
    if report["case_reports"] != case_reports:
        raise RuntimeError("Patch Proposal Controlled Follow-up Rehearsal report case summary drifted")
    report["schema_files"] = [
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-operator-signal"]["patch-proposal-controlled-follow-up-rehearsal-receipt.json"]
    )
        ...
        for filename, payload in artifacts.items():
            ...
            if load_json(path) != payload:
                raise SystemExit(
                    ...
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_rehearsal.py --write"
                )
...
def html_report(markdown: str) -> str:
    ...
    return (
        ...
        "<html lang=\"en\">\n"
        "<meta charset=\"utf-8\">\n"
        "<title>Patch Proposal Controlled Follow-up Rehearsal</title>\n"
        "<body><pre>"
        + escaped
        + "</pre></body></html>\n"
    )
...
def main() -> int:
    ...
    if args.check:
        ...
        if not REPORT.is_file() or REPORT.read_text(encoding="utf-8") != report_text:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Rehearsal report is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_rehearsal.py --write"
            )
        if not MARKDOWN_REPORT.is_file() or MARKDOWN_REPORT.read_text(encoding="utf-8") != md:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Rehearsal markdown is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_rehearsal.py --write"
            )
        if not HTML_REPORT.is_file() or HTML_REPORT.read_text(encoding="utf-8") != html:
            raise SystemExit(
                "Patch Proposal Controlled Follow-up Rehearsal HTML is out of date. "
                "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_rehearsal.py --write"
            )
        print("ok    Patch Proposal Controlled Follow-up Rehearsal report is up to date")
        return 0
