...
import patch_proposal_customer_feedback_delivery_trust_case_bridge as case_bridge  # noqa: E402


RECEIPT_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-boundary-receipt-v1"
ENVELOPE_REFS_SCHEMA_VERSION = "patch-proposal-controlled-follow-up-envelope-refs-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-controlled-follow-up-boundary-gate-v1"
RECONSTRUCTION_SCHEMA_VERSION = "patch-proposal-follow-up-boundary-reconstruction-v1"
...
class PatchProposalControlledFollowUpBoundaryGateError(ValueError):
...
def load_json(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(payload, dict):
        raise PatchProposalControlledFollowUpBoundaryGateError(f"Expected JSON object: {path}")
    return payload
...
def _require_object(payload: Mapping[str, Any], key: str, *, label: str) -> Mapping[str, Any]:
    ...
    if not isinstance(value, Mapping):
        raise PatchProposalControlledFollowUpBoundaryGateError(f"{label}.{key} must be an object")
    return value
...
def _validate_privacy(payload: Mapping[str, Any], *, label: str) -> None:
    ...
    if forbidden:
        raise PatchProposalControlledFollowUpBoundaryGateError(
...
def source_bridge_receipt(case_id: str) -> dict[str, Any] | None:
    ...
    receipt = artifacts.get("patch-proposal-delivery-trust-case-bridge-receipt.json")
...
def source_handoff_refs(case_id: str) -> dict[str, Any] | None:
    artifacts = source_bridge_artifacts(case_id)
    refs = artifacts.get("patch-proposal-delivery-trust-case-handoff-refs.json")
...
def validate_reconstruction(reconstruction: Mapping[str, Any]) -> dict[str, Any]:
    if reconstruction.get("schema_version") != RECONSTRUCTION_SCHEMA_VERSION:
        raise PatchProposalControlledFollowUpBoundaryGateError("reconstruction schema_version drifted")
    ...
    if source_type not in ALLOWED_RECONSTRUCTION_SOURCES:
        raise PatchProposalControlledFollowUpBoundaryGateError("unsupported reconstruction source")
    if reconstruction.get("passive_attention_only") is not False:
        raise PatchProposalControlledFollowUpBoundaryGateError("passive attention is insufficient")
    if reconstruction.get("active_checkpoint_count") != len(REQUIRED_CHECKPOINT_IDS):
        raise PatchProposalControlledFollowUpBoundaryGateError("active checkpoint count drifted")
    if reconstruction.get("strong_evidence_count") != len(REQUIRED_CHECKPOINT_IDS):
        raise PatchProposalControlledFollowUpBoundaryGateError("strong evidence count drifted")
    checkpoints = reconstruction.get("checkpoints")
    if not isinstance(checkpoints, list):
        raise PatchProposalControlledFollowUpBoundaryGateError("reconstruction checkpoints must be a list")
    checkpoint_ids = set()
    for item in checkpoints:
        if not isinstance(item, Mapping):
            raise PatchProposalControlledFollowUpBoundaryGateError("reconstruction checkpoint must be an object")
        if item.get("raw_answer_included") is not False:
            raise PatchProposalControlledFollowUpBoundaryGateError("checkpoint must not include raw answers")
        if item.get("evidence_strength") != "strong":
            raise PatchProposalControlledFollowUpBoundaryGateError("checkpoint evidence must be strong")
        checkpoint_ids.add(str(item.get("checkpoint_id")))
    if checkpoint_ids != REQUIRED_CHECKPOINT_IDS:
        raise PatchProposalControlledFollowUpBoundaryGateError("required reconstruction checkpoints missing")
    ...
    refs = {
        **_base(ENVELOPE_REFS_SCHEMA_VERSION),
        ...
        "source_bridge_receipt_ref": _artifact_ref(
            ...
            f"fixtures/patch-proposal-customer-feedback-delivery-trust-case-bridge/{_source_case_id(case_id)}/patch-proposal-delivery-trust-case-bridge-receipt.json",
            bridge_receipt,
        ),
        "source_handoff_refs_ref": _artifact_ref(
            "patch-proposal-delivery-trust-case-handoff-refs",
            f"fixtures/patch-proposal-customer-feedback-delivery-trust-case-bridge/{_source_case_id(case_id)}/patch-proposal-delivery-trust-case-handoff-refs.json",
            handoff_refs,
        ),
...
def validate_envelope_refs(refs: Mapping[str, Any]) -> dict[str, Any]:
    if refs.get("schema_version") != ENVELOPE_REFS_SCHEMA_VERSION:
        raise PatchProposalControlledFollowUpBoundaryGateError("envelope refs schema_version drifted")
    ...
    for key in ("source_bridge_receipt_ref", "source_handoff_refs_ref", "active_reconstruction_ref", "follow_up_envelope_ref"):
        ...
        if ref.get("raw_body_included") is not False:
            raise PatchProposalControlledFollowUpBoundaryGateError(f"{key} must not include raw body")
    for key in ("source_evidence_refs", "delivery_trust_refs"):
        values = refs.get(key)
        if not isinstance(values, list) or not values:
            raise PatchProposalControlledFollowUpBoundaryGateError(f"envelope refs missing {key}")
        for item in values:
            if not isinstance(item, Mapping):
                raise PatchProposalControlledFollowUpBoundaryGateError(f"{key} item must be object")
            if item.get("raw_body_included") is not False:
                raise PatchProposalControlledFollowUpBoundaryGateError(f"{key} must not include raw bodies")
    ...
    for key in (
        ...
        if summary.get(key) is not False:
            raise PatchProposalControlledFollowUpBoundaryGateError(f"harness_summary.{key} must be false")
    if summary.get("status") != "ready_for_controlled_follow_up_envelope_refs":
        raise PatchProposalControlledFollowUpBoundaryGateError("envelope refs status drifted")
    return dict(refs)
    ...
    *,
    ...
    boundary_receipt = {
        **_base(RECEIPT_SCHEMA_VERSION),
        ...
        "source_refs": {
            "bridge_receipt_ref": (
                f"fixtures/patch-proposal-customer-feedback-delivery-trust-case-bridge/"
                f"{_source_case_id(case_id)}/patch-proposal-delivery-trust-case-bridge-receipt.json"
            ),
            "handoff_refs_ref": (
                f"fixtures/patch-proposal-customer-feedback-delivery-trust-case-bridge/"
                f"{_source_case_id(case_id)}/patch-proposal-delivery-trust-case-handoff-refs.json"
            ),
...
def validate_boundary_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    if receipt.get("schema_version") != RECEIPT_SCHEMA_VERSION:
        raise PatchProposalControlledFollowUpBoundaryGateError("receipt schema_version drifted")
    ...
    if status not in {"follow_up_envelope_refs_ready", "blocked"}:
        raise PatchProposalControlledFollowUpBoundaryGateError("boundary receipt status is invalid")
    if not isinstance(reasons, list):
        raise PatchProposalControlledFollowUpBoundaryGateError("boundary receipt blocked_reasons must be a list")
    ...
    if status == "follow_up_envelope_refs_ready":
        if decision != "prepare_metadata_only_follow_up_envelope_refs":
            raise PatchProposalControlledFollowUpBoundaryGateError("ready receipt decision drifted")
        if reasons:
            raise PatchProposalControlledFollowUpBoundaryGateError("ready receipt must not include blocked reasons")
        if not isinstance(refs, Mapping):
            raise PatchProposalControlledFollowUpBoundaryGateError("ready receipt must include envelope refs")
        validate_envelope_refs(refs)
    else:
        if decision != "block_controlled_follow_up_boundary":
            raise PatchProposalControlledFollowUpBoundaryGateError("blocked receipt decision drifted")
        if not reasons:
            raise PatchProposalControlledFollowUpBoundaryGateError("blocked receipt must include reasons")
        if refs is not None:
            raise PatchProposalControlledFollowUpBoundaryGateError("blocked receipt must not include envelope refs")
    ...
    for key in (
        ...
        if policy.get(key) is not False:
            raise PatchProposalControlledFollowUpBoundaryGateError(f"harness_policy.{key} must be false")
    if policy.get("emit_refs_only") is not True:
        raise PatchProposalControlledFollowUpBoundaryGateError("boundary gate must emit refs only")
    return dict(receipt)


def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    if case_id not in CASE_IDS:
        raise PatchProposalControlledFollowUpBoundaryGateError(f"Unknown case id: {case_id}")
    ...
    boundary_receipt = build_boundary_receipt(
        ...
        customer_visible_send_requested=case_id == "blocked-automatic-customer-send",
    ...
    artifacts = {"patch-proposal-controlled-follow-up-boundary-receipt.json": boundary_receipt}
    if isinstance(reconstruction, Mapping):
        artifacts["patch-proposal-follow-up-boundary-reconstruction.json"] = dict(reconstruction)
    envelope_refs = boundary_receipt.get("envelope_refs")
    if isinstance(envelope_refs, Mapping):
        artifacts["patch-proposal-controlled-follow-up-envelope-refs.json"] = validate_envelope_refs(envelope_refs)
    return artifacts
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_boundary_receipt(
            cases[case_id]["patch-proposal-controlled-follow-up-boundary-receipt.json"]
        )
        case_reports.append(
            {
                "case_id": case_id,
...
def main() -> int:
    ...
    if args.bridge_receipt:
        ...
        selected: dict[str, dict[str, dict[str, Any]]] = {
            "custom": {"patch-proposal-controlled-follow-up-boundary-receipt.json": receipt}
        }
        envelope_refs = receipt.get("envelope_refs")
        if isinstance(envelope_refs, Mapping):
            selected["custom"]["patch-proposal-controlled-follow-up-envelope-refs.json"] = validate_envelope_refs(
                envelope_refs
            )
    else:
        ...
        dump_json(
            {
                ...
                "ready_case_count": sum(
                    ...
                    if artifacts["patch-proposal-controlled-follow-up-boundary-receipt.json"]["status"]
                    == "follow_up_envelope_refs_ready"
                ),
                "blocked_case_count": sum(
                    1
                    for artifacts in selected.values()
                    if artifacts["patch-proposal-controlled-follow-up-boundary-receipt.json"]["status"] == "blocked"
                ),
                "model_calls_performed": False,
                "raw_follow_up_body_included": False,
                "customer_visible_send_performed": False,
