#!/usr/bin/env python3
"""Gate Patch Proposal Product Loop brief candidates into Product Loop runs.

This layer consumes `patch-proposal-product-loop-brief-candidate-v1` artifacts
from the Patch Proposal Customer Feedback Spec/Eval Authoring Gate. It can emit
...
import patch_proposal_customer_feedback_spec_eval_authoring_gate as authoring_gate  # noqa: E402


RECEIPT_SCHEMA_VERSION = "patch-proposal-customer-feedback-product-loop-brief-intake-receipt-v1"
REPORT_SCHEMA_VERSION = "patch-proposal-customer-feedback-product-loop-brief-intake-gate-v1"

DEFAULT_OUTPUT_DIR = ROOT / ".cognitive-loop" / "artifacts" / "patch-proposal-customer-feedback-product-loop-brief-intake-gate"
...
CLAIM_BOUNDARY = {
    "current_claim": (
        "A Patch Proposal Product Loop brief candidate can become metadata-only "
        "Product Loop scenario/run candidates only after active developer/product-loop "
        "boundary reconstruction. The transition stops before Delivery Trust Harness "
        "invocation and cannot perform customer-visible, source, or production effects."
    ),
    "not_claimed": [
...
def _base(schema_version: str) -> dict[str, Any]:
    return {
        ...
        "isolation": dict(authoring_gate.owner_gate.backlog_bridge.ISOLATION),
...
def _validate_privacy(payload: Mapping[str, Any], *, label: str) -> None:
    ...
    for key, expected in authoring_gate.owner_gate.backlog_bridge.ISOLATION.items():
...
def source_authoring_receipt(case_id: str) -> dict[str, Any]:
    return source_authoring_artifacts(case_id)["patch-proposal-customer-feedback-spec-eval-authoring-receipt.json"]
...
def build_product_loop_candidate(
    ...
    *,
    ...
    scenario["scenario_id"] = f"patch-proposal-product-loop-brief-intake-scenario-{candidate_hash[:16]}"
    scenario["source"] = {
        "source_type": "patch_proposal_product_loop_brief_candidate",
    ...
    scenario["claim_boundary"] = {
        "current_claim": (
            ...
            "Patch Proposal Product Loop brief candidate, not raw spec/eval/customer bodies."
        ),
        "not_claimed": [
...
def build_intake_receipt(
    ...
    *,
    ...
    if candidate_payload is None:
        reasons.append("patch_proposal_brief_candidate_missing")
    else:
        try:
            validated_candidate = validate_source_brief_candidate(candidate_payload)
        except Exception:
            reasons.append("patch_proposal_brief_candidate_invalid")
    ...
    receipt = {
        **_base(RECEIPT_SCHEMA_VERSION),
        "intake_receipt_id": (
            f"patch-proposal-product-loop-brief-intake-{source_hash[:16]}"
            if source_hash
            else f"patch-proposal-product-loop-brief-intake-blocked-{case_id}"
        ),
        "case_id": case_id,
        ...
        "decision": (
            ...
            else "block_patch_proposal_product_loop_brief_intake_gate"
        ),
        "blocked_reasons": reasons,
        "source_refs": {
            "authoring_receipt_ref": (
                f"fixtures/patch-proposal-customer-feedback-spec-eval-authoring-gate/"
...
def validate_intake_receipt(receipt: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if status == "created_product_loop_scenario_run_candidate":
        if decision != "create_patch_proposal_product_loop_scenario_run_candidate":
            raise PatchProposalCustomerFeedbackProductLoopBriefIntakeGateError(
                "created receipt must create Patch Proposal Product Loop scenario/run candidate"
            )
        if reasons:
            raise PatchProposalCustomerFeedbackProductLoopBriefIntakeGateError(
        ...
        if scenario_payload["source"]["source_type"] != "patch_proposal_product_loop_brief_candidate":
            raise PatchProposalCustomerFeedbackProductLoopBriefIntakeGateError(
                "scenario source must be Patch Proposal Product Loop brief candidate"
            )
        ...
        if decision != "block_patch_proposal_product_loop_brief_intake_gate":
            raise PatchProposalCustomerFeedbackProductLoopBriefIntakeGateError(
...
def build_case_artifacts(case_id: str) -> dict[str, dict[str, Any]]:
    ...
    if case_id == "blocked-missing-brief-candidate":
        candidate = None
        receipt_payload = None
    if case_id == "blocked-invalid-brief-candidate" and candidate is not None:
        candidate = dict(candidate)
        candidate["next_boundary"] = FORBIDDEN_NEXT_BOUNDARY
    ...
    artifacts = {"patch-proposal-product-loop-brief-intake-receipt.json": receipt}
...
def build_report(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> dict[str, Any]:
    ...
    for case_id in CASE_IDS:
        receipt = validate_intake_receipt(cases[case_id]["patch-proposal-product-loop-brief-intake-receipt.json"])
        case_reports.append(
            {
                "case_id": case_id,
    ...
    report = {
        **_base(REPORT_SCHEMA_VERSION),
        "status": "pass",
        "purpose": (
            "Prove Patch Proposal Product Loop brief candidates can create metadata-only Product Loop "
            "scenario/run candidates only after active developer/product-loop reconstruction, without "
            "AI-review-only promotion, Delivery Trust Harness invocation, customer follow-up, source "
            "mutation, production mutation, secrets, or model credentials."
        ),
        "source_chain": {
...
def main() -> int:
    ...
    if args.brief_candidate:
        ...
        selected = {
            "custom": {
                "patch-proposal-product-loop-brief-intake-receipt.json": build_intake_receipt(
                    "custom",
    ...
    print(
        dump_json(
            {
                "schema_version": "patch-proposal-customer-feedback-product-loop-brief-intake-cli-result-v1",
                "status": "ok",
                "case_ids": list(selected),
                "created_case_count": sum(
                    1
                    for artifacts in selected.values()
                    if artifacts["patch-proposal-product-loop-brief-intake-receipt.json"]["status"]
                    == "created_product_loop_scenario_run_candidate"
                ),
                "blocked_case_count": sum(
                    1
                    for artifacts in selected.values()
                    if artifacts["patch-proposal-product-loop-brief-intake-receipt.json"]["status"] == "blocked"
                ),
                "model_calls_performed": False,
