#!/usr/bin/env python3
"""Verify controlled follow-up feedback Product Loop Brief Intake Gate artifacts."""
...
import patch_proposal_customer_feedback_controlled_follow_up_feedback_product_loop_brief_intake_gate as gate  # noqa: E402


FIXTURE_ROOT = ROOT / "fixtures" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-gate"
REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-gate.json"
MARKDOWN_REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-gate.md"
HTML_REPORT = ROOT / "platform" / "generated" / "study-anything-patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-gate.html"
RECEIPT_SCHEMA_FILE = ROOT / "platform" / "schemas" / "cbb" / "patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt-v1.schema.json"
REPORT_SCHEMA_FILE = ROOT / "platform" / "schemas" / "cbb" / "patch-proposal-customer-feedback-controlled-follow-up-feedback-product-loop-brief-intake-gate-v1.schema.json"
...
def validate_case(case_id: str, artifacts: Mapping[str, Mapping[str, Any]]) -> dict[str, Any]:
    receipt = gate.validate_intake_receipt(
        artifacts["patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt.json"]
    )
    ...
    dual_loop.assert_metadata_only(receipt, label=f"patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake:{case_id}")
    return {
        "case_id": case_id,
...
def verify_cli(cases: Mapping[str, Mapping[str, Mapping[str, Any]]]) -> None:
    with tempfile.TemporaryDirectory(prefix="study-anything-patch-product-loop-intake-") as tmp:
        ...
        proc = subprocess.run(
            [
                sys.executable,
                str(ROOT / "scripts" / "patch_proposal_customer_feedback_controlled_follow_up_feedback_product_loop_brief_intake_gate.py"),
                "--case",
                "all",
                "--output-dir",
                str(output_dir),
        ...
        dual_loop.assert_metadata_only(
            stdout,
            label="patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-cli",
        )
        ...
        for case_id, expected_artifacts in cases.items():
            ...
            if expected_artifacts["patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt.json"]["status"] == "blocked":
...
def build_report() -> dict[str, Any]:
    ...
    report["negative_fixtures"] = negative_fixtures(
        cases["pass-customer-signal"]["patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt.json"]
    )
        ...
        for filename, payload in artifacts.items():
            ...
            if load_json(path) != payload:
                raise SystemExit(
                    ...
                    "Run: python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_product_loop_brief_intake_gate.py --write"
                )
        scenario_path = case_dir / "product-loop-scenario.json"
        run_path = case_dir / "product-loop-run.json"
        if (
            artifacts["patch-proposal-controlled-follow-up-feedback-product-loop-brief-intake-receipt.json"]["status"] == "blocked"
