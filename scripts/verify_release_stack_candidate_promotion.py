...
PR_227_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-227-intake-candidate.json"
PR_228_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-228-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.117-v0.3.118"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.115-v0.3.116"
...
def expected_group(pr_227_source: Mapping[str, Any], pr_228_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=227,
                order=1,
                evidence_refs=PR_227_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=228,
                order=2,
                evidence_refs=PR_228_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_227_source: Mapping[str, Any],
    pr_228_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [225, 226]:
        raise ReleaseStackPromotionError("previous current group must retain PR #225-#226 audit rows.")

    expected = expected_group(pr_227_source, pr_228_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #227/#228 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_227_source, pr_228_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_227_source, pr_228_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_227_source: Mapping[str, Any], pr_228_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_227_source, pr_228_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_227_source, pr_228_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-227-source", type=Path, default=PR_227_SOURCE)
    parser.add_argument("--pr-228-source", type=Path, default=PR_228_SOURCE)
...
def main() -> None:
    ...
    pr_227_source = load_json(args.pr_227_source)
    pr_228_source = load_json(args.pr_228_source)
    report = build_report(manifest, pr_227_source, pr_228_source)
