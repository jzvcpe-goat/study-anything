...
PR_195_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-195-intake-candidate.json"
PR_196_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-196-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.85-v0.3.86"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.83-v0.3.84"
...
def expected_group(pr_195_source: Mapping[str, Any], pr_196_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=195,
                order=1,
                evidence_refs=PR_195_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=196,
                order=2,
                evidence_refs=PR_196_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_195_source: Mapping[str, Any],
    pr_196_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [193, 194]:
        raise ReleaseStackPromotionError("previous current group must retain PR #193-#194 audit rows.")

    expected = expected_group(pr_195_source, pr_196_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #195/#196 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_195_source, pr_196_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_195_source, pr_196_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_195_source: Mapping[str, Any], pr_196_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_195_source, pr_196_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_195_source, pr_196_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-195-source", type=Path, default=PR_195_SOURCE)
    parser.add_argument("--pr-196-source", type=Path, default=PR_196_SOURCE)
...
def main() -> None:
    ...
    pr_195_source = load_json(args.pr_195_source)
    pr_196_source = load_json(args.pr_196_source)
    report = build_report(manifest, pr_195_source, pr_196_source)
