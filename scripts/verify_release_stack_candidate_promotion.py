...
PR_233_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-233-intake-candidate.json"
PR_234_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-234-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.123-v0.3.124"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.121-v0.3.122"
...
def expected_group(pr_233_source: Mapping[str, Any], pr_234_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=233,
                order=1,
                evidence_refs=PR_233_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=234,
                order=2,
                evidence_refs=PR_234_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_233_source: Mapping[str, Any],
    pr_234_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [231, 232]:
        raise ReleaseStackPromotionError("previous current group must retain PR #231-#232 audit rows.")

    expected = expected_group(pr_233_source, pr_234_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #233/#234 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_233_source, pr_234_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_233_source, pr_234_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_233_source: Mapping[str, Any], pr_234_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_233_source, pr_234_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_233_source, pr_234_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-233-source", type=Path, default=PR_233_SOURCE)
    parser.add_argument("--pr-234-source", type=Path, default=PR_234_SOURCE)
...
def main() -> None:
    ...
    pr_233_source = load_json(args.pr_233_source)
    pr_234_source = load_json(args.pr_234_source)
    report = build_report(manifest, pr_233_source, pr_234_source)
