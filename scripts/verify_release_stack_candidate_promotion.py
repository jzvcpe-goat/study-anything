...
PR_203_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-203-intake-candidate.json"
PR_204_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-204-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.93-v0.3.94"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.91-v0.3.92"
...
def expected_group(pr_203_source: Mapping[str, Any], pr_204_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=203,
                order=1,
                evidence_refs=PR_203_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=204,
                order=2,
                evidence_refs=PR_204_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_203_source: Mapping[str, Any],
    pr_204_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [201, 202]:
        raise ReleaseStackPromotionError("previous current group must retain PR #201-#202 audit rows.")

    expected = expected_group(pr_203_source, pr_204_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #203/#204 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_203_source, pr_204_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_203_source, pr_204_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_203_source: Mapping[str, Any], pr_204_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_203_source, pr_204_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_203_source, pr_204_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-203-source", type=Path, default=PR_203_SOURCE)
    parser.add_argument("--pr-204-source", type=Path, default=PR_204_SOURCE)
...
def main() -> None:
    ...
    pr_203_source = load_json(args.pr_203_source)
    pr_204_source = load_json(args.pr_204_source)
    report = build_report(manifest, pr_203_source, pr_204_source)
