...
PR_211_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-211-intake-candidate.json"
PR_212_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-212-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.101-v0.3.102"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.99-v0.3.100"
...
def expected_group(pr_211_source: Mapping[str, Any], pr_212_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=211,
                order=1,
                evidence_refs=PR_211_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=212,
                order=2,
                evidence_refs=PR_212_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_211_source: Mapping[str, Any],
    pr_212_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [209, 210]:
        raise ReleaseStackPromotionError("previous current group must retain PR #209-#210 audit rows.")

    expected = expected_group(pr_211_source, pr_212_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #211/#212 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_211_source, pr_212_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_211_source, pr_212_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_211_source: Mapping[str, Any], pr_212_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_211_source, pr_212_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_211_source, pr_212_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-211-source", type=Path, default=PR_211_SOURCE)
    parser.add_argument("--pr-212-source", type=Path, default=PR_212_SOURCE)
...
def main() -> None:
    ...
    pr_211_source = load_json(args.pr_211_source)
    pr_212_source = load_json(args.pr_212_source)
    report = build_report(manifest, pr_211_source, pr_212_source)
