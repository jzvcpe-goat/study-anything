...
PR_277_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-277-intake-candidate.json"
PR_278_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-278-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.44-v0.3.45"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.148-v0.3.149"
...
def expected_group(pr_277_source: Mapping[str, Any], pr_278_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "group_id": PROMOTED_GROUP_ID,
        "role": "current",
        "status": "completed",
        "target_branch": "main",
        "summary": "Completed Dual Loop delivery trust and Customer Handoff self-intake chain.",
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=277,
                order=1,
                evidence_refs=PR_277_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=278,
                order=2,
                evidence_refs=PR_278_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_277_source: Mapping[str, Any],
    pr_278_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [258, 259]:
        raise ReleaseStackPromotionError("previous current group must retain PR #258-#259 audit rows.")

    expected = expected_group(pr_277_source, pr_278_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #277/#278 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_277_source, pr_278_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_277_source, pr_278_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_277_source: Mapping[str, Any], pr_278_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_277_source, pr_278_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_277_source, pr_278_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-277-source", type=Path, default=PR_277_SOURCE)
    parser.add_argument("--pr-278-source", type=Path, default=PR_278_SOURCE)
...
def main() -> None:
    ...
    pr_277_source = load_json(args.pr_277_source)
    pr_278_source = load_json(args.pr_278_source)
    report = build_report(manifest, pr_277_source, pr_278_source)
