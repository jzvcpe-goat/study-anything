...
PR_SOURCES = {
    300: ROOT / "fixtures" / "release-stack" / "pr-300-intake-candidate.json",
}
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.172"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.170"
...
def expected_group(pr_sources: Mapping[int, Mapping[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "status": "completed",
        "target_branch": "main",
        "summary": "Completed self-intake for the Code Review Delivery Class handoff release evidence chain.",
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=300,
                order=1,
                evidence_refs=PR_EVIDENCE_REFS[300],
...
def verify_promoted_manifest(
    ...
    if previous_prs != [298]:
        raise ReleaseStackPromotionError("previous current group must retain PR #298 audit rows.")

    expected = expected_group(pr_sources)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #300 candidate group.")
...
def build_report(manifest: dict[str, Any], pr_sources: Mapping[int, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    report = {
        ...
        "source_reports": [
            "fixtures/release-stack/pr-300-intake-candidate.json",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-300-source", type=Path, default=PR_SOURCES[300])
...
def main() -> None:
    ...
    pr_sources = {
        300: load_json(args.pr_300_source),
    }
