...
PR_205_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-205-intake-candidate.json"
PR_206_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-206-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.95-v0.3.96"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.93-v0.3.94"
...
def expected_group(pr_205_source: Mapping[str, Any], pr_206_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=205,
                order=1,
                evidence_refs=PR_205_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=206,
                order=2,
                evidence_refs=PR_206_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_205_source: Mapping[str, Any],
    pr_206_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [203, 204]:
        raise ReleaseStackPromotionError("previous current group must retain PR #203-#204 audit rows.")

    expected = expected_group(pr_205_source, pr_206_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #205/#206 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_205_source, pr_206_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_205_source, pr_206_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_205_source: Mapping[str, Any], pr_206_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_205_source, pr_206_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_205_source, pr_206_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-205-source", type=Path, default=PR_205_SOURCE)
    parser.add_argument("--pr-206-source", type=Path, default=PR_206_SOURCE)
...
def main() -> None:
    ...
    pr_205_source = load_json(args.pr_205_source)
    pr_206_source = load_json(args.pr_206_source)
    report = build_report(manifest, pr_205_source, pr_206_source)
