...
PR_231_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-231-intake-candidate.json"
PR_232_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-232-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.121-v0.3.122"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.119-v0.3.120"
...
def expected_group(pr_231_source: Mapping[str, Any], pr_232_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=231,
                order=1,
                evidence_refs=PR_231_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=232,
                order=2,
                evidence_refs=PR_232_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_231_source: Mapping[str, Any],
    pr_232_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [229, 230]:
        raise ReleaseStackPromotionError("previous current group must retain PR #229-#230 audit rows.")

    expected = expected_group(pr_231_source, pr_232_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #231/#232 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_231_source, pr_232_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_231_source, pr_232_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_231_source: Mapping[str, Any], pr_232_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_231_source, pr_232_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_231_source, pr_232_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-231-source", type=Path, default=PR_231_SOURCE)
    parser.add_argument("--pr-232-source", type=Path, default=PR_232_SOURCE)
...
def main() -> None:
    ...
    pr_231_source = load_json(args.pr_231_source)
    pr_232_source = load_json(args.pr_232_source)
    report = build_report(manifest, pr_231_source, pr_232_source)
