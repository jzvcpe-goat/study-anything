...
PR_246_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-246-intake-candidate.json"
PR_247_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-247-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.136-v0.3.137"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.133-v0.3.134"
...
def expected_group(pr_246_source: Mapping[str, Any], pr_247_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=246,
                order=1,
                evidence_refs=PR_246_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=247,
                order=2,
                evidence_refs=PR_247_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_246_source: Mapping[str, Any],
    pr_247_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [243, 244]:
        raise ReleaseStackPromotionError("previous current group must retain PR #243-#244 audit rows.")

    expected = expected_group(pr_246_source, pr_247_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #246/#247 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_246_source, pr_247_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_246_source, pr_247_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_246_source: Mapping[str, Any], pr_247_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_246_source, pr_247_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_246_source, pr_247_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-246-source", type=Path, default=PR_246_SOURCE)
    parser.add_argument("--pr-247-source", type=Path, default=PR_247_SOURCE)
...
def main() -> None:
    ...
    pr_246_source = load_json(args.pr_246_source)
    pr_247_source = load_json(args.pr_247_source)
    report = build_report(manifest, pr_246_source, pr_247_source)
