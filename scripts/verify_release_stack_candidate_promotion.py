...
PR_258_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-258-intake-candidate.json"
PR_259_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-259-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.148-v0.3.149"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.145-v0.3.146"
...
def expected_group(pr_258_source: Mapping[str, Any], pr_259_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=258,
                order=1,
                evidence_refs=PR_258_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=259,
                order=2,
                evidence_refs=PR_259_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_258_source: Mapping[str, Any],
    pr_259_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [255, 256]:
        raise ReleaseStackPromotionError("previous current group must retain PR #255-#256 audit rows.")

    expected = expected_group(pr_258_source, pr_259_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #258/#259 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_258_source, pr_259_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_258_source, pr_259_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_258_source: Mapping[str, Any], pr_259_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_258_source, pr_259_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_258_source, pr_259_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-258-source", type=Path, default=PR_258_SOURCE)
    parser.add_argument("--pr-259-source", type=Path, default=PR_259_SOURCE)
...
def main() -> None:
    ...
    pr_258_source = load_json(args.pr_258_source)
    pr_259_source = load_json(args.pr_259_source)
    report = build_report(manifest, pr_258_source, pr_259_source)
