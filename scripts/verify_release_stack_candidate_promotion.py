...
PR_213_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-213-intake-candidate.json"
PR_214_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-214-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.103-v0.3.104"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.101-v0.3.102"
...
def expected_group(pr_213_source: Mapping[str, Any], pr_214_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=213,
                order=1,
                evidence_refs=PR_213_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=214,
                order=2,
                evidence_refs=PR_214_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_213_source: Mapping[str, Any],
    pr_214_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [211, 212]:
        raise ReleaseStackPromotionError("previous current group must retain PR #211-#212 audit rows.")

    expected = expected_group(pr_213_source, pr_214_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #213/#214 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_213_source, pr_214_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_213_source, pr_214_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_213_source: Mapping[str, Any], pr_214_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_213_source, pr_214_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_213_source, pr_214_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-213-source", type=Path, default=PR_213_SOURCE)
    parser.add_argument("--pr-214-source", type=Path, default=PR_214_SOURCE)
...
def main() -> None:
    ...
    pr_213_source = load_json(args.pr_213_source)
    pr_214_source = load_json(args.pr_214_source)
    report = build_report(manifest, pr_213_source, pr_214_source)
