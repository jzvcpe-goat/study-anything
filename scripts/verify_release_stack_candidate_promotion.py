...
PR_193_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-193-intake-candidate.json"
PR_194_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-194-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.83-v0.3.84"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.81-v0.3.82"
...
def expected_group(pr_193_source: Mapping[str, Any], pr_194_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=193,
                order=1,
                evidence_refs=PR_193_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=194,
                order=2,
                evidence_refs=PR_194_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_193_source: Mapping[str, Any],
    pr_194_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [191, 192]:
        raise ReleaseStackPromotionError("previous current group must retain PR #191-#192 audit rows.")

    expected = expected_group(pr_193_source, pr_194_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #193/#194 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_193_source, pr_194_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_193_source, pr_194_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_193_source: Mapping[str, Any], pr_194_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_193_source, pr_194_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_193_source, pr_194_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-193-source", type=Path, default=PR_193_SOURCE)
    parser.add_argument("--pr-194-source", type=Path, default=PR_194_SOURCE)
...
def main() -> None:
    ...
    pr_193_source = load_json(args.pr_193_source)
    pr_194_source = load_json(args.pr_194_source)
    report = build_report(manifest, pr_193_source, pr_194_source)
