...
PR_SOURCES = {
    290: ROOT / "fixtures" / "release-stack" / "pr-290-intake-candidate.json",
}
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.162"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.160"
...
def expected_group(pr_sources: Mapping[int, Mapping[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "status": "completed",
        "target_branch": "main",
        "summary": "Completed self-intake for the Dual Loop Trust Scenario Pack release asset chain.",
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=290,
                order=1,
                evidence_refs=PR_EVIDENCE_REFS[290],
...
def verify_promoted_manifest(
    ...
    if previous_prs != [288]:
        raise ReleaseStackPromotionError("previous current group must retain PR #288 audit rows.")

    expected = expected_group(pr_sources)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #290 candidate group.")
...
def build_report(manifest: dict[str, Any], pr_sources: Mapping[int, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    report = {
        ...
        "source_reports": [
            "fixtures/release-stack/pr-290-intake-candidate.json",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-290-source", type=Path, default=PR_SOURCES[290])
...
def main() -> None:
    ...
    pr_sources = {
        290: load_json(args.pr_290_source),
    }
