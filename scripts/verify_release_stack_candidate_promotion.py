...
PR_215_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-215-intake-candidate.json"
PR_216_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-216-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.105-v0.3.106"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.103-v0.3.104"
...
def expected_group(pr_215_source: Mapping[str, Any], pr_216_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=215,
                order=1,
                evidence_refs=PR_215_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=216,
                order=2,
                evidence_refs=PR_216_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_215_source: Mapping[str, Any],
    pr_216_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [213, 214]:
        raise ReleaseStackPromotionError("previous current group must retain PR #213-#214 audit rows.")

    expected = expected_group(pr_215_source, pr_216_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #215/#216 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_215_source, pr_216_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_215_source, pr_216_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_215_source: Mapping[str, Any], pr_216_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_215_source, pr_216_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_215_source, pr_216_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-215-source", type=Path, default=PR_215_SOURCE)
    parser.add_argument("--pr-216-source", type=Path, default=PR_216_SOURCE)
...
def main() -> None:
    ...
    pr_215_source = load_json(args.pr_215_source)
    pr_216_source = load_json(args.pr_216_source)
    report = build_report(manifest, pr_215_source, pr_216_source)
