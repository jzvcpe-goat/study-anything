...
PR_243_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-243-intake-candidate.json"
PR_244_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-244-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.133-v0.3.134"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.130-v0.3.131"
...
def expected_group(pr_243_source: Mapping[str, Any], pr_244_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=243,
                order=1,
                evidence_refs=PR_243_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=244,
                order=2,
                evidence_refs=PR_244_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_243_source: Mapping[str, Any],
    pr_244_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [240, 241]:
        raise ReleaseStackPromotionError("previous current group must retain PR #240-#241 audit rows.")

    expected = expected_group(pr_243_source, pr_244_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #243/#244 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_243_source, pr_244_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_243_source, pr_244_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_243_source: Mapping[str, Any], pr_244_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_243_source, pr_244_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_243_source, pr_244_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-243-source", type=Path, default=PR_243_SOURCE)
    parser.add_argument("--pr-244-source", type=Path, default=PR_244_SOURCE)
...
def main() -> None:
    ...
    pr_243_source = load_json(args.pr_243_source)
    pr_244_source = load_json(args.pr_244_source)
    report = build_report(manifest, pr_243_source, pr_244_source)
