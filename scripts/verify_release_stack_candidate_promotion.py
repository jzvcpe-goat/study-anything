...
PR_235_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-235-intake-candidate.json"
PR_236_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-236-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.125-v0.3.126"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.123-v0.3.124"
...
def expected_group(pr_235_source: Mapping[str, Any], pr_236_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=235,
                order=1,
                evidence_refs=PR_235_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=236,
                order=2,
                evidence_refs=PR_236_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_235_source: Mapping[str, Any],
    pr_236_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [233, 234]:
        raise ReleaseStackPromotionError("previous current group must retain PR #233-#234 audit rows.")

    expected = expected_group(pr_235_source, pr_236_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #235/#236 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_235_source, pr_236_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_235_source, pr_236_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_235_source: Mapping[str, Any], pr_236_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_235_source, pr_236_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_235_source, pr_236_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-235-source", type=Path, default=PR_235_SOURCE)
    parser.add_argument("--pr-236-source", type=Path, default=PR_236_SOURCE)
...
def main() -> None:
    ...
    pr_235_source = load_json(args.pr_235_source)
    pr_236_source = load_json(args.pr_236_source)
    report = build_report(manifest, pr_235_source, pr_236_source)
