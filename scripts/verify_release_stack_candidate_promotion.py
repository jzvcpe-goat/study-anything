...
PR_221_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-221-intake-candidate.json"
PR_222_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-222-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.111-v0.3.112"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.109-v0.3.110"
...
def expected_group(pr_221_source: Mapping[str, Any], pr_222_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=221,
                order=1,
                evidence_refs=PR_221_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=222,
                order=2,
                evidence_refs=PR_222_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_221_source: Mapping[str, Any],
    pr_222_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [219, 220]:
        raise ReleaseStackPromotionError("previous current group must retain PR #219-#220 audit rows.")

    expected = expected_group(pr_221_source, pr_222_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #221/#222 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_221_source, pr_222_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_221_source, pr_222_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_221_source: Mapping[str, Any], pr_222_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_221_source, pr_222_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_221_source, pr_222_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-221-source", type=Path, default=PR_221_SOURCE)
    parser.add_argument("--pr-222-source", type=Path, default=PR_222_SOURCE)
...
def main() -> None:
    ...
    pr_221_source = load_json(args.pr_221_source)
    pr_222_source = load_json(args.pr_222_source)
    report = build_report(manifest, pr_221_source, pr_222_source)
