...
PR_249_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-249-intake-candidate.json"
PR_250_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-250-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.139-v0.3.140"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.136-v0.3.137"
...
def expected_group(pr_249_source: Mapping[str, Any], pr_250_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=249,
                order=1,
                evidence_refs=PR_249_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=250,
                order=2,
                evidence_refs=PR_250_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_249_source: Mapping[str, Any],
    pr_250_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [246, 247]:
        raise ReleaseStackPromotionError("previous current group must retain PR #246-#247 audit rows.")

    expected = expected_group(pr_249_source, pr_250_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #249/#250 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_249_source, pr_250_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_249_source, pr_250_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_249_source: Mapping[str, Any], pr_250_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_249_source, pr_250_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_249_source, pr_250_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-249-source", type=Path, default=PR_249_SOURCE)
    parser.add_argument("--pr-250-source", type=Path, default=PR_250_SOURCE)
...
def main() -> None:
    ...
    pr_249_source = load_json(args.pr_249_source)
    pr_250_source = load_json(args.pr_250_source)
    report = build_report(manifest, pr_249_source, pr_250_source)
