...
PR_185_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-185-intake-candidate.json"
PR_186_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-186-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.75-v0.3.76"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.73-v0.3.74"
...
def expected_group(pr_185_source: Mapping[str, Any], pr_186_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "group_id": PROMOTED_GROUP_ID,
        "role": "current",
        "status": "completed",
        "target_branch": "main",
        "summary": "Completed release stack maintenance chain for candidate promotion and self-intake continuity.",
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=185,
                order=1,
                evidence_refs=PR_185_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=186,
                order=2,
                evidence_refs=PR_186_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_185_source: Mapping[str, Any],
    pr_186_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [183, 184]:
        raise ReleaseStackPromotionError("previous current group must retain PR #183-#184 audit rows.")

    expected = expected_group(pr_185_source, pr_186_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #185/#186 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_185_source, pr_186_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_185_source, pr_186_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_185_source: Mapping[str, Any], pr_186_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_185_source, pr_186_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_185_source, pr_186_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-185-source", type=Path, default=PR_185_SOURCE)
    parser.add_argument("--pr-186-source", type=Path, default=PR_186_SOURCE)
...
def main() -> None:
    ...
    pr_185_source = load_json(args.pr_185_source)
    pr_186_source = load_json(args.pr_186_source)
    report = build_report(manifest, pr_185_source, pr_186_source)
