...
PR_197_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-197-intake-candidate.json"
PR_198_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-198-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.87-v0.3.88"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.85-v0.3.86"
...
def expected_group(pr_197_source: Mapping[str, Any], pr_198_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=197,
                order=1,
                evidence_refs=PR_197_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=198,
                order=2,
                evidence_refs=PR_198_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_197_source: Mapping[str, Any],
    pr_198_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [195, 196]:
        raise ReleaseStackPromotionError("previous current group must retain PR #195-#196 audit rows.")

    expected = expected_group(pr_197_source, pr_198_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #197/#198 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_197_source, pr_198_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_197_source, pr_198_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_197_source: Mapping[str, Any], pr_198_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_197_source, pr_198_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_197_source, pr_198_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-197-source", type=Path, default=PR_197_SOURCE)
    parser.add_argument("--pr-198-source", type=Path, default=PR_198_SOURCE)
...
def main() -> None:
    ...
    pr_197_source = load_json(args.pr_197_source)
    pr_198_source = load_json(args.pr_198_source)
    report = build_report(manifest, pr_197_source, pr_198_source)
