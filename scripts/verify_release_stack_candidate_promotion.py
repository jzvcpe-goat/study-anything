...
PR_SOURCES = {
    304: ROOT / "fixtures" / "release-stack" / "pr-304-intake-candidate.json",
}
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.176"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.174"
...
def expected_group(pr_sources: Mapping[int, Mapping[str, Any]]) -> dict[str, Any]:
    return {
        ...
        "status": "completed",
        "target_branch": "main",
        "summary": "Completed self-intake for the Delivery Class Registry release evidence chain.",
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=304,
                order=1,
                evidence_refs=PR_EVIDENCE_REFS[304],
...
def verify_promoted_manifest(
    ...
    if previous_prs != [302]:
        raise ReleaseStackPromotionError("previous current group must retain PR #302 audit rows.")

    expected = expected_group(pr_sources)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #304 candidate group.")
...
def build_report(manifest: dict[str, Any], pr_sources: Mapping[int, Mapping[str, Any]]) -> dict[str, Any]:
    ...
    report = {
        ...
        "source_reports": [
            "fixtures/release-stack/pr-304-intake-candidate.json",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-304-source", type=Path, default=PR_SOURCES[304])
...
def main() -> None:
    ...
    pr_sources = {
        304: load_json(args.pr_304_source),
    }
