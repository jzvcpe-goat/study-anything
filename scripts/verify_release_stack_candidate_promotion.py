...
PR_229_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-229-intake-candidate.json"
PR_230_SOURCE = ROOT / "fixtures" / "release-stack" / "pr-230-intake-candidate.json"
REPORT_SCHEMA_VERSION = "release-stack-candidate-promotion-v1"
PROMOTED_GROUP_ID = "release-stack-promotion-v0.3.119-v0.3.120"
PREVIOUS_CURRENT_GROUP_ID = "release-stack-promotion-v0.3.117-v0.3.118"
...
def expected_group(pr_229_source: Mapping[str, Any], pr_230_source: Mapping[str, Any]) -> dict[str, Any]:
    return {
        ...
        "stack": [
            load_source_row(
                ...
                expected_pr=229,
                order=1,
                evidence_refs=PR_229_EVIDENCE_REFS,
            ...
            load_source_row(
                ...
                expected_pr=230,
                order=2,
                evidence_refs=PR_230_EVIDENCE_REFS,
...
def verify_promoted_manifest(
    ...
    pr_229_source: Mapping[str, Any],
    pr_230_source: Mapping[str, Any],
) -> dict[str, Any]:
    ...
    if previous_prs != [227, 228]:
        raise ReleaseStackPromotionError("previous current group must retain PR #227-#228 audit rows.")

    expected = expected_group(pr_229_source, pr_230_source)
    actual = find_group(manifest, PROMOTED_GROUP_ID)
    if actual != expected:
        raise ReleaseStackPromotionError("promoted current group does not match the expected #229/#230 candidate group.")
...
def run_negative_case(
    ...
    mutator(payload)
    try:
        verify_promoted_manifest(payload, pr_229_source, pr_230_source)
...
def negative_fixtures(
    ...
    return [run_negative_case(case_id, mutator, manifest, pr_229_source, pr_230_source) for case_id, mutator in cases]


def build_report(manifest: dict[str, Any], pr_229_source: Mapping[str, Any], pr_230_source: Mapping[str, Any]) -> dict[str, Any]:
    readiness = verify_promoted_manifest(manifest, pr_229_source, pr_230_source)
    ...
    report = {
        ...
        "negative_fixtures": negative_fixtures(manifest, pr_229_source, pr_230_source),
        "privacy": {
            "metadata_only": True,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--pr-229-source", type=Path, default=PR_229_SOURCE)
    parser.add_argument("--pr-230-source", type=Path, default=PR_230_SOURCE)
...
def main() -> None:
    ...
    pr_229_source = load_json(args.pr_229_source)
    pr_230_source = load_json(args.pr_230_source)
    report = build_report(manifest, pr_229_source, pr_230_source)
