...
The first local validator for these contracts is now available in `scripts/verify_cognitive_loop_contracts.py`. A companion local CLI can initialize the contracts and render a static HTML DecisionCard artifact with `python3 scripts/cognitive_loop_cli.py report --html`. Mastra, watchers, and the full realtime HTML console are still planned layers.
...
第一版本地 validator 已经在 `scripts/verify_cognitive_loop_contracts.py` 中可用。配套本地 CLI 可以初始化契约，并通过 `python3 scripts/cognitive_loop_cli.py report --html` 渲染静态 HTML DecisionCard artifact；Mastra、watcher 和完整实时 HTML console 仍然是计划中的层。
