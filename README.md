...
计划中表面：常驻项目 watcher、生产级 runtime gate、通用 HTML Artifact console、个人插件，以及后续 hosted/team 服务。当前桥接能力：一个可复制到外部 Mastra 项目的 Mastra adapter contract pack、只含 metadata 的 runtime dry-run harness、位于 `platform/mastra-runtime/` 的最小 repo-started Mastra runtime MVP、本地 libSQL 持久化 suspend/resume 证明，以及面向 metadata-only ProjectEvent 的手动 watcher ingest。
