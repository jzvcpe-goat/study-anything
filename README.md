...
- `TrustEvidenceHandoffPack`: portable metadata-only pack, documented in `docs/trust-evidence-handoff-pack.md`, that lets external operators or customer reviewers inspect delivery classes, scenario decisions, claim boundaries, and privacy rules from a ZIP.
...
- `TrustEvidenceHandoffPack`：可携带、只含 metadata 的交付信任证据包，详见 `docs/trust-evidence-handoff-pack.md`；让外部操作者或客户审阅者能从 ZIP 检查交付类、场景决策、claim boundary 和隐私规则。
...
see `docs/security.md`, `docs/self-hosting.md`, `docs/self-host-reliability.md`, and
`docs/scheduled-reliability.md`. The scheduled workflow also emits a metadata-only longitudinal
index that keeps short diagnostics separate from strict default-duration evidence.
