...
GENERATOR_SPEC = importlib.util.spec_from_file_location(
    "generate_platform_public_support_status",
    GENERATOR,
)
...
class PlatformPublicSupportStatusTests(unittest.TestCase):
    def test_generator_failure_formatter_is_actionable_and_redacted(self) -> None:
        ...
        temp_path = "/private/" + "tmp/study-anything/public-support-status.json"
        message = generator.format_cli_failure(
            RuntimeError(
                f"public support status stale at {temp_path} "
                f"with Authorization: Bearer {secret}"
            )
        )

        self.assertIn("generate_platform_public_support_status failed:", message)
        self.assertIn("Next steps:", message)
        self.assertIn("generate_platform_public_support_status.py --check", message)
