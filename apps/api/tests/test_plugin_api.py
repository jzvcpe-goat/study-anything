...
class PluginApiTests(unittest.TestCase):
    ...
    def test_preview_plugin_returns_permission_details_without_installing(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(body["path"], "<local-plugin-path>")
            self.assertEqual(body["install_dir"], "<plugin-install-dir>")
            self.assertEqual(body["quarantine_dir"], "<plugin-quarantine-dir>")
    ...
    def test_install_plugin_quarantines_after_permission_confirmation(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertFalse(body["entrypoints_executed"])
            self.assertEqual(body["path"], "<local-plugin-path>")
            self.assertEqual(body["destination_dir"], "<plugin-quarantine-dir>")
            self.assertNotIn(str(root), quarantined.text)
    ...
    def test_install_plugin_copies_only_after_explicit_approval(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(body["path"], "<local-plugin-path>")
            self.assertEqual(body["destination_dir"], "<plugin-install-dir>")
            self.assertNotIn(str(root), installed.text)
