    ...
    return EvidenceBundleV1(
        ...
        evidence=[
            ...
            EvidenceItemV1(
                ...
                supported_scope=sandbox_supported_scope,
                metadata={
                    "contained": contained,
        ...
        maximum_supported_scope=DeliveryScope.SANDBOX_ONLY,
        claim_boundary=ClaimBoundaryV1(
            current_claim=(
                "The mapped v0 evidence bundle cannot exceed sandbox-only; "
                "individual evidence states still gate delivery."
            ),
...
def dual_loop_and_delivery_to_gate_decision(
    ...
    if gate["status"] != "allowed" and decision.status == "allow":
        raise CompatibilityMappingError("canonical kernel expanded a blocked v0 gate")
    return decision


def add_v0_decision_evidence(
    evidence_bundle: EvidenceBundleV1,
    gate_receipt: Mapping[str, Any],
    delivery_receipt: Mapping[str, Any],
) -> EvidenceBundleV1:
    gate = dual_loop.validate_gate_receipt(gate_receipt)
    receipt = delivery_trust.validate_delivery_trust_receipt(delivery_receipt)
    gate_passed = gate["status"] == "allowed"
    gate_missing = any(
        str(reason).endswith("_missing") for reason in gate.get("reasons", [])
    )
    delivery_passed = receipt["status"] == "allowed"
    controlled_scope_supported = (
        evidence_bundle.maximum_supported_scope == DeliveryScope.SANDBOX_ONLY
        and gate_passed
        and delivery_passed
    )
    maximum_scope = (
        DeliveryScope.CONTROLLED_CUSTOMER_HANDOFF
        if controlled_scope_supported
        else evidence_bundle.maximum_supported_scope
    )
    ...
    evidence.extend(
        [
            EvidenceItemV1(
                ...
                evidence_type="dual_loop_gate",
                status=("passed" if gate_passed else "missing" if gate_missing else "failed"),
                source_schema_version=dual_loop.DUAL_LOOP_GATE_RECEIPT_SCHEMA_VERSION,
            ...
            EvidenceItemV1(
                ...
                evidence_type="v0_delivery_trust_receipt",
                status="passed" if delivery_passed else "failed",
                source_schema_version=delivery_trust.DELIVERY_TRUST_RECEIPT_SCHEMA_VERSION,
                source_ref="delivery-trust-receipt.json",
                supported_scope=(
                    DeliveryScope.CONTROLLED_CUSTOMER_HANDOFF
                    if delivery_passed
                    else DeliveryScope.BLOCKED
                ),
                metadata={"compatibility_source_only": True},
            ),
        ]
    )
    return EvidenceBundleV1(
        ...
        subject_ref=evidence_bundle.subject_ref,
        policy_ref=evidence_bundle.policy_ref,
        evidence=evidence,
        maximum_supported_scope=maximum_scope,
        claim_boundary=ClaimBoundaryV1(
            current_claim=(
                "The complete mapped v0 chain supports controlled customer handoff only."
                if controlled_scope_supported
                else "The mapped v0 chain supports no more than its sandbox evidence."
            ),
            maximum_scope=maximum_scope,
            not_claimed=[
                "production approval",
                "portable signed attestation",
                "customer outcome guarantee",
                "authority beyond the source v0 receipts",
            ],
        ),
        privacy=evidence_bundle.privacy,
...
def map_v0_delivery_chain(
    ...
    *,
    ...
    evidence = add_v0_decision_evidence(
        evidence,
        gate_receipt,
        delivery_receipt,
    )
