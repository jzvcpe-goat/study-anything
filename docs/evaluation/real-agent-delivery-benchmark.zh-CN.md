...
每个审核项首先展示一份不可省略的“交付说明”：交付 Agent 与执行模型、
候选代码补丁、任务标题、来源仓库、变更规模、接收者、使用目的、责任主体、
最大范围以及“待人工复核，尚未放行”状态。真实 Agent packet 缺少这些字段，
或交付说明与候选物不一致时，审核台失败关闭，不要求人类在主体不明的情况下作答。

审核台不会先要求用户给自己选择职位标签。在展示候选证据前，服务器会先问三个
具体问题：本次承担什么决定、能够独立理解哪些材料、准备把交付推进到哪里。
系统据此推荐通俗、产品负责人、软件工程师、安全与审计或协议原文视图；如果全文
代码复核能力不足，或者目标超过 `personal_local`，则先阻断并说明需要转交的角色。
最终提交还必须携带本轮 preparation token，不能通过直接调用本地接口绕过提问。

用户在通过路由后仍可切换展示，但切换只改变语言和关注重点，不改变底层 packet、
题目顺序、正确答案、证据或权限，也不证明审核者具备相应专业资格。每种展示都会
明确说明：

1. 当前审核的是哪一份 Agent 候选补丁，而不是整个仓库或模型总体能力；
2. 当前用户需要判断哪些交付边界；
3. 哪些技术结论不应由该背景用户猜测；
4. 何时必须选择“无法判断，需要转交对应专业人员”。

协议机器值使用确定性的中文语义表解释。GitHub 任务原文和代码补丁不做自动语义
翻译，因为当前仓库没有证据证明通用翻译能够保留专业含义。全文参考复核仍要求
相应代码审查能力；不具备该能力的用户可以完成边界重构，但不能据此替代工程复核。
收据只写入派生的审核路由、最终展示模式和提问策略 digest，不保存原始问卷选择，
同时保持 `professional_qualification_claimed: false`。
