...
- `python3 scripts/verify_cognitive_loop_pr_ci_receipt.py --check` creates metadata-only PR CI Receipt evidence for required checks, from offline fixtures today and sanitized `gh pr checks --json` output later.
- PR CI Receipt Lite rejects missing required checks, stale head SHA, malformed gh-json, secret-like text, raw logs, unsafe commands, and policy weakening; it never reads GitHub tokens or job logs.
