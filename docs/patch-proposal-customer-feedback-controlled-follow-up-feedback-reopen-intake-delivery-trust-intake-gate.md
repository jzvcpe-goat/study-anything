...
This gate is the Patch Proposal controlled follow-up feedback reopen-intake
bridge from Product Loop evidence into the Delivery Trust case candidate queue.

It consumes the Product Loop brief intake receipt, scenario, and run artifacts
emitted by the controlled follow-up feedback reopen-intake Product Loop Brief
Intake Gate. It requires controlled-failure, attention-reconstruction, and Dual
Loop gate evidence before emitting a metadata-only
`patch-proposal-delivery-trust-case-candidate-v1`.
...
- missing or blocked Product Loop intake receipts;
- missing Product Loop scenarios;
- missing or blocked Product Loop runs;
- missing reopen-intake chain refs;
- missing failure contracts;
- missing sandbox receipts;
- missing attention reconstruction;
- missing Dual Loop gate receipts;
...
```text
fixtures/patch-proposal-customer-feedback-controlled-follow-up-feedback-reopen-intake-delivery-trust-intake-gate/
```
...
```bash
python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_reopen_intake_delivery_trust_intake_gate.py --check
```

The verifier checks deterministic fixtures, schema identity, CLI roundtrip,
custom Product Loop run input paired with the deterministic reopen-intake
receipt/scenario bridge, metadata-only privacy boundaries, Dual Loop evidence
requirements, and negative injections for raw delivery case bodies, customer
handoff package creation, Delivery Trust harness invocation, automatic
