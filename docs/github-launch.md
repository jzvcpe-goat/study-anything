...
API_BASE=http://127.0.0.1:8000 AGENT_ENDPOINT=http://mock-http-agent:8787/invoke python3 scripts/verify_mock_http_agent_flow.py
...
- `scripts/verify_published_image_launch.py --tag v0.3.29-alpha` can pull the public API image,
...
- `docs/release-notes/v0.3.29-alpha.md` lists known limitations.
...
git switch main
git pull --ff-only
git tag v0.3.29-alpha
git push origin v0.3.29-alpha
```

Create the prerelease after the tag is pushed:

```bash
gh release create v0.3.29-alpha \
  --prerelease \
  --title "Study Anything v0.3.29-alpha" \
  --notes-file docs/release-notes/v0.3.29-alpha.md
```
...
Use `docs/release-notes/v0.3.29-alpha.md` as the GitHub Release body. Keep the matching file in the
