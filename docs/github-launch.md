...
- `scripts/verify_backup_restore_drill.py` can create, mutate, restore, and clean up a disposable
  Docker stack.
- `scripts/verify_published_image_launch.py --tag v0.2.9-alpha` can pull the public API image,
  verify the running API version, and complete the API learning loop.
- `docs/release-notes/v0.2.9-alpha.md` lists known limitations.
...
```bash
gh release create v0.2.9-alpha \
  --prerelease \
  --title "Study Anything v0.2.9-alpha" \
  --notes-file docs/release-notes/v0.2.9-alpha.md
```
...
```bash
docker manifest inspect ghcr.io/jzvcpe-goat/study-anything/api:v0.2.9-alpha
python3 scripts/verify_published_image_launch.py --tag v0.2.9-alpha
```

## Release Notes

Use `docs/release-notes/v0.2.9-alpha.md` as the GitHub Release body. Keep the matching file in the
