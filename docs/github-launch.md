...
- `scripts/verify_published_image_launch.py --tag v0.3.22-alpha` can pull the public API image,
...
- `docs/release-notes/v0.3.22-alpha.md` lists known limitations.
...
git switch main
git pull --ff-only
git tag v0.3.22-alpha
git push origin v0.3.22-alpha
```

Create the prerelease after the tag is pushed:

```bash
gh release create v0.3.22-alpha \
  --prerelease \
  --title "Study Anything v0.3.22-alpha" \
  --notes-file docs/release-notes/v0.3.22-alpha.md
```
...
```bash
docker manifest inspect ghcr.io/jzvcpe-goat/study-anything/api:v0.3.22-alpha
...
Use `docs/release-notes/v0.3.22-alpha.md` as the GitHub Release body. Keep the matching file in the
