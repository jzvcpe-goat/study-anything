...
- `scripts/verify_backup_restore_drill.py` can create, mutate, restore, and clean up a disposable
  Docker stack.
- `scripts/verify_published_image_launch.py --tag v0.2.8-alpha` can pull the public API image,
  verify the running API version, and complete the API learning loop.
- `docs/release-notes/v0.2.8-alpha.md` lists known limitations.
...
git switch main
git pull --ff-only
git tag v0.2.8-alpha
git push origin v0.2.8-alpha
```

Create the prerelease after the tag is pushed:

```bash
gh release create v0.2.8-alpha \
  --prerelease \
  --title "Study Anything v0.2.8-alpha" \
  --notes-file docs/release-notes/v0.2.8-alpha.md
```
...
```bash
docker manifest inspect ghcr.io/jzvcpe-goat/study-anything/api:v0.2.8-alpha
python3 scripts/verify_published_image_launch.py --tag v0.2.8-alpha
```

## Release Notes

Use `docs/release-notes/v0.2.8-alpha.md` as the GitHub Release body. Keep the matching file in the
