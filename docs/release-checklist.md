...
- [ ] `.venv/bin/python scripts/verify_hosted_identity_tenancy.py --check`
- [ ] `.venv/bin/python scripts/generate_external_security_audit_pack.py --check`
- [ ] `.venv/bin/python scripts/verify_external_security_audit_pack.py --check`
- [ ] `.venv/bin/python scripts/verify_self_host_soak.py --check`
- [ ] `.venv/bin/python scripts/verify_self_host_reliability_matrix.py --check`
...
- [ ] `.venv/bin/python scripts/generate_platform_plugin_downloads.py --check`
- [ ] `.venv/bin/python scripts/verify_platform_plugin_downloads.py --check`
...
- [ ] Run `python3 scripts/verify_hosted_identity_tenancy.py --check` and verify it returns `schema_version=hosted-identity-tenancy-verification-v1`, requires signed short-lived OIDC JWTs, binds principals to issuer + tenant + subject, ignores body identity spoofing, hides cross-tenant sessions/workspaces with `404`, enforces workspace RBAC, principal-scopes Agent providers, blocks unscoped hosted routes, and includes no tokens, JWKS, raw identity claims, Agent endpoints, secrets, or local paths.
- [ ] Run `python3 scripts/verify_external_security_audit_pack.py --check` and verify it returns `schema_version=external-security-audit-pack-verification-v1`, `status=pass`, package status `ready_for_independent_audit`, seven scope areas, one safe ZIP root, matching source/archive hashes, a required external human reviewer and signed report, `audit_completed=false`, and rejection of self-certified audit-pass, secret, local-path, or unsafe-archive inputs.
...
- [ ] Run `python3 scripts/generate_release_asset_adoption.py --check` and `python3 scripts/verify_release_asset_adoption.py --fixture fixtures/release-asset-adoption/asset-only-pass.json --asset-dir platform/generated --runtime metadata-only`; verify `release-asset-adoption-v1` covers GitHub Release zip assets, plugin-pack manifests, sha256 sidecars, adoption-pack manifest hashes, embedded published-image evidence, `release-asset-adoption-fixture-v1`, and no source text, answers, prompts, Agent endpoints, model keys, support bundle private payloads, or local absolute paths.
- [ ] Verify the GitHub prerelease attaches `study-anything-codex-plugin-pack.json`, `study-anything-codex-plugin-pack.zip`, `study-anything-codex-plugin-pack.sha256`, `study-anything-kimi-plugin-pack.json`, `study-anything-kimi-plugin-pack.zip`, `study-anything-kimi-plugin-pack.sha256`, `study-anything-workbuddy-plugin-pack.json`, `study-anything-workbuddy-plugin-pack.zip`, `study-anything-workbuddy-plugin-pack.sha256`, `study-anything-hermes-plugin-pack.json`, `study-anything-hermes-plugin-pack.zip`, `study-anything-hermes-plugin-pack.sha256`, `study-anything-dual-loop-trust-scenario-pack.json`, `study-anything-dual-loop-trust-scenario-pack.zip`, `study-anything-dual-loop-trust-scenario-pack.sha256`, and `study-anything-dual-loop-trust-pack-consumer-walkthrough.json`.
...
- [ ] Verify `GET /v1/commercial/readiness` returns `schema_version=commercial-readiness-v1`, `github_oss_launch=ready`, `platform_agent_distribution=ready`, `hosted_paid_services=not_ready`, `security_audit_status=ready_for_independent_audit`, `security_audit_completed=false`, all local core invariants passing, and no source text, answers, endpoints, model secrets, billing credentials, or raw contacts.
...
- [ ] Verify `python3 scripts/verify_release_stack_intake_candidate.py --check` returns `release-stack-intake-candidate-v1`, `status=pass`, and produces a metadata-only candidate/applied group for the latest completed release-stack PR without storing GitHub tokens, job logs, annotations, artifacts, or live payloads; verify `python3 scripts/verify_release_stack_intake_candidate.py --from-gh-pr 193 --report-only` before promotion when live GitHub metadata is available.
- [ ] Verify `python3 scripts/verify_release_stack_candidate_promotion.py --check` returns `release-stack-candidate-promotion-v1`, `status=pass`, archives the previous current group, promotes the manifest-driven current PR group, keeps top-level stack mirroring the current group, rejects duplicate PRs, unsafe commands, secret/log/artifact payloads, manifest regressions, and missing or failed required checks.
