...
- [ ] After creating the GitHub prerelease, run `python3 scripts/verify_release_asset_adoption.py --tag v0.3.25-alpha --runtime metadata-only` and verify it emits `release-asset-adoption-proof-v1`.
...
- [ ] After GHCR publish, run `python3 scripts/verify_published_image_launch.py --tag v0.3.25-alpha`.
- [ ] If local GHCR pulls are too slow, run `python3 scripts/verify_published_image_launch.py --tag v0.3.25-alpha --pull-timeout-seconds 180 --allow-pull-timeout-report` and pair the JSON diagnostic with a successful `docker manifest inspect ghcr.io/jzvcpe-goat/study-anything/api:v0.3.25-alpha`.
