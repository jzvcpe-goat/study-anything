...
- [ ] Verify `python3 scripts/verify_release_stack_intake_candidate.py --check` returns `release-stack-intake-candidate-v1`, `status=pass`, and produces a metadata-only candidate group for the latest completed release-stack PR without mutating `platform/release-stack.json`; verify `python3 scripts/verify_release_stack_intake_candidate.py --from-gh-pr 183 --report-only` before promotion when live GitHub metadata is available.
