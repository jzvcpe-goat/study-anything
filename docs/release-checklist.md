...
- [ ] Verify `python3 scripts/verify_release_stack_candidate_promotion.py --check` returns `release-stack-candidate-promotion-v1`, `status=pass`, archives the previous current group, promotes PR #197/#198 into the current group, keeps top-level stack mirroring the current group, rejects duplicate PRs, unsafe commands, secret/log/artifact payloads, manifest regressions, and missing or failed required checks.
