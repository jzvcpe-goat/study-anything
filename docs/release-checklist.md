...
- [ ] Verify `python3 scripts/verify_release_stack_intake_candidate.py --check` returns `release-stack-intake-candidate-v1`, `status=pass`, and produces a metadata-only candidate/applied group for the latest completed release-stack PR without storing GitHub tokens, job logs, annotations, artifacts, or live payloads; verify `python3 scripts/verify_release_stack_intake_candidate.py --from-gh-pr 187 --report-only` before promotion when live GitHub metadata is available.
- [ ] Verify `python3 scripts/verify_release_stack_candidate_promotion.py --check` returns `release-stack-candidate-promotion-v1`, `status=pass`, archives the previous current group, promotes PR #187/#188 into the current group, keeps top-level stack mirroring the current group, rejects duplicate PRs, unsafe commands, secret/log/artifact payloads, manifest regressions, and missing or failed required checks.
