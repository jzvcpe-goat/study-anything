...
Patch Proposal Customer Feedback Controlled Follow-up Feedback Outcome Receipt sits after
...
- consume only a ready `patch-proposal-controlled-follow-up-feedback-rehearsal-receipt-v1`;
...
```bash
python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_outcome.py --check
```

Regenerate:

```bash
python3 scripts/verify_patch_proposal_customer_feedback_controlled_follow_up_feedback_outcome.py --write
```

## Chinese Summary

Patch Proposal Customer Feedback Controlled Follow-up Feedback Outcome Receipt 是
Controlled Follow-up Feedback Rehearsal 之后的一层。
