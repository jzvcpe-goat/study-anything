...
- GitHub workflow template: `platform/workflows/cognitive-loop-review-agent-manual.yml` is a copy-ready manual GitHub Actions template for validating a report or existing acceptance bundle, running the metadata-only policy gate, and writing a metadata-only Checks/step summary. It is `workflow_dispatch` only, exposes `advisory` / `soft` / `strict` policy choices, does not call real models, does not require external Agent secrets, and does not upload the raw report. `python3 scripts/verify_cognitive_loop_review_agent_github_workflow.py --check` proves this workflow and its unsafe fixture.
- Policy gate: after the acceptance bundle is valid, run `python3 scripts/cognitive_loop_review_agent_policy_gate.py --bundle-dir /tmp/review-agent-acceptance --policy soft` to convert metadata-only Review Agent evidence into an auditable exit code. Policies are `advisory`, `soft`, and `strict`; `python3 scripts/verify_cognitive_loop_review_agent_policy_gate.py --check` proves decision-path coverage and privacy boundaries.
- Workflow install smoke: `python3 scripts/verify_cognitive_loop_review_agent_workflow_install_smoke.py --check` proves an external adopter can extract the adoption pack, copy the workflow into `.github/workflows/`, and reproduce metadata-only `advisory` / `soft` / `strict` policy gate behavior without a repo checkout, raw report upload, or real model call.
- Adoption drill: `python3 scripts/verify_cognitive_loop_review_agent_adoption_drill.py --check` proves the zip-only external adopter path end to end: extract the adoption pack, validate embedded Review Agent evidence, build acceptance bundles, validate bilingual PR comment packs, run the policy matrix, and install the manual workflow without reintroducing raw diff or report-body material.
...
- GitHub workflow 模板：`platform/workflows/cognitive-loop-review-agent-manual.yml` 是可复制的手动 GitHub Actions 模板，用来校验 report 或已有 acceptance bundle、运行 metadata-only policy gate，并写出 metadata-only Checks/step summary。它只允许 `workflow_dispatch`，暴露 `advisory` / `soft` / `strict` 策略选择，不调用真实模型，不需要外部 Agent secret，也不会上传 raw report。`python3 scripts/verify_cognitive_loop_review_agent_github_workflow.py --check` 负责验证该 workflow 和 unsafe 负例 fixture。
- Policy gate：acceptance bundle 验证通过后，运行 `python3 scripts/cognitive_loop_review_agent_policy_gate.py --bundle-dir /tmp/review-agent-acceptance --policy soft`，把 metadata-only Review Agent evidence 转成可审计退出码。策略包括 `advisory`、`soft` 和 `strict`；`python3 scripts/verify_cognitive_loop_review_agent_policy_gate.py --check` 验证决策覆盖和隐私边界。
- Workflow 安装验收：`python3 scripts/verify_cognitive_loop_review_agent_workflow_install_smoke.py --check` 证明外部采用者可以解压 adoption pack，把 workflow 复制到 `.github/workflows/`，并在不 checkout 仓库、不上传 raw report、不调用真实模型的情况下复现 metadata-only `advisory` / `soft` / `strict` policy gate 行为。
- Adoption drill：`python3 scripts/verify_cognitive_loop_review_agent_adoption_drill.py --check` 证明 zip-only 外部采用路径端到端成立：解压 adoption pack、校验内置 Review Agent 证据、生成 acceptance bundle、校验中英文 PR comment pack、运行 policy matrix，并安装手动 workflow，同时不重新引入 raw diff 或 report 正文。
