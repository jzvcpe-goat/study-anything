...
Study Anything v0.3.29-alpha uses a GitHub-first support desk for external platform adoption. The
...
The machine-readable evidence is `platform-support-triage-v1` plus
...
python3 scripts/verify_platform_support_bundle_replay.py \
  --pack platform/generated/study-anything-platform-adoption-pack.zip
```
...
When a user attaches a redacted bundle or cleanroom report, replay it locally:

```bash
python3 scripts/replay_support_bundle.py --bundle support-bundle.json --issue-body
```

The replay emits `platform-support-bundle-replay-v1` with a stable classification, maintainer action,
recommended next steps, and a copyable issue body. It accepts:

- `platform-support-bundle-v1`
- `release-cleanroom-bootstrap-v1`
- published-image launch reports that include `manifest_evidence`

Privacy-unsafe bundles are blocked before triage. For example, the mock
`fixtures/platform-support-bundles/privacy-contract-violation.json` must exit with a privacy-blocking
classification instead of producing a public support path.
...
`scripts/release_check.sh` runs the support triage and support bundle replay commands. A release is
not ready if the report, replay fixtures, issue templates, ticket fixtures, platform packs, ecosystem
submission metadata, adoption pack, or docs drift from the support desk contract.

## Onboarding Readiness And SLA

v0.3.29-alpha adds `platform-onboarding-readiness-v1` on top of support triage. It proves first
...
v0.3.29-alpha adds `public-support-status-v1` and `public-maintainer-dashboard-v1`. This layer is
...
v0.3.29-alpha adds `adopter-evidence-archive-v1` for support handoff. Before closing a release or
...
Attach the archive checksum, public status link, and relevant `adopter-evidence-fixture-v1` id. Keep
the handoff metadata-only: no raw learning data, private support payloads, Agent endpoint secrets,
model keys, personal profiles, or browser/video/app private context.
