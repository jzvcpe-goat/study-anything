...
Use this pack for Hermes Agent when you want Hermes to remain the conversation, model, memory, and tool surface while Study Anything runs as the local learning and Cognitive Loop engine.

This pack has two current entrypoints:

1. Hermes Skill install: install `skills/study-anything/SKILL.md` into Hermes so the Agent knows when and how to call the local runtime.
2. Local HTTP/CLI tools: start Study Anything locally, then let Hermes run the CLI or call a private/local API endpoint through your own tool setup.

It is not yet a published Hermes-native Python plugin repo. Do not claim `hermes plugins install jzvcpe-goat/study-anything` is supported until a standalone Hermes plugin with a root `plugin.yaml` is released and field-tested.

## Install The Skill

From a released GitHub ref, Hermes can install a direct `SKILL.md` URL:
...
For a local checkout, either copy or symlink `skills/study-anything` into `~/.hermes/skills/`, or add the repo `skills` directory as an external skill directory in your Hermes config.

Then start the local runtime and run the same verification path used by the other platform packs:
...
Hermes owns the real model choice, user conversation, memory, browser or app tools, MCP servers, and any external credentials. Study Anything owns source-bound learning, mastery, local receipts, redacted eval evidence, Obsidian/NotebookLM-style handoff, and Cognitive Loop metadata artifacts.

Future Hermes-native plugin work should be released as a standalone plugin repository that Hermes can install with `hermes plugins install owner/repo`. That later plugin can register typed tools or commands, but this pack intentionally stays metadata-only and local-first.

---

# Shared HTTP Tool Pack Reference

Use this pack for Hermes-compatible agent workspaces that can import HTTP tools or OpenAPI specs.
...
The verifier emits `adoption-proof-v1` and proves the Hermes-compatible HTTP tool path without
...
Hermes-compatible platform Agent HTTP workspace can call the local API and local Cognitive Loop
...
For a Hermes workspace that owns a Mastra runtime, expose `platform/mastra/` as a static adapter
...
`platform/generated/study-anything-cognitive-loop-skill-entrypoint.json`. Verify the Hermes
...
Schemas for static Hermes validation of the recipe CLI success, receipt, and failure reports plus
PR CI receipt/source metadata.
`platform/schemas/cognitive-loop-pr-ci-receipt.schema.json` and
`platform/schemas/cognitive-loop-pr-ci-source.schema.json` are standalone schemas Hermes can
validate without running Python, reading GitHub tokens, fetching job logs, or mutating source files.
...
`platform/generated/study-anything-platform-handoff-checklist.json` gives Hermes-compatible operators a
release handoff checklist for import, verification, runtime choice, and support escalation.
`platform/generated/study-anything-launch-acceptance-ledger.json` gives Hermes-compatible operators the
aggregated launch acceptance state and current commercial boundary.
`platform/generated/study-anything-github-launch-operator-guide.json` gives Hermes-compatible operators
the GitHub release sequence, required release assets, and local-first launch boundary.

For code-review acceptance, the Hermes-compatible workspace can pass a temporary handoff request to its
...
manual `workflow_dispatch` workflow after the external Hermes report path is clear. It writes a
...
The workflow applies the captured policy exit code after artifact upload so Hermes evidence is
...
`POST /v1/sessions/{session_id}/context-package` when Hermes has collected browser pages,
